...
public sealed record AlpineRidgeNativeLockedChestCandidateRequest(
...
public sealed record AlpineRidgeNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record AlpineRidgeNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<AlpineRidgeNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record AlpineRidgeNativeLockedChestCandidateResult(
    string OutputImagePath,
    AlpineRidgeNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Alpine Ridge transplant of Peace Keepers' native Key + Locked Chest
/// source bundle. Alpine Ridge already owns the retail Key dispatch, class 0x000D
...
/// </summary>
public static class AlpineRidgeNativeLockedChestCandidateExporter
{
    public const string RecipeId = "alpineridge.peacekeepers.nativeLockedChest.disposable.v1";
    ...
    private const int AlpineRidgeOverlayEntryIndex = 35;
    private const long AlpineRidgeOverlayEntryOffset = 0x29F9800;
    private const int OriginalOverlaySize = 0x13800;
    private const int ExpandedOverlaySize = 0x14000;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x8008DEC0;
    private const uint RelocatedCopyBufferAddress = 0x8008E600;

    private const int AlpineRidgeDataEntryIndex = 36;
    private const long AlpineRidgeDataEntryOffset = 0x2A0D000;
    private const int OriginalDataEntrySize = 0x29F800;
    private const int ExpandedDataEntrySize = 0x2A1000;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x16E000;
    private const int OriginalActorSubfileSize = 0x6D000;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1DB000;
    private const int ExpandedSceneOffset = 0x1DC800;
    private const int SceneSize = 0x12000;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const int OverlayPayloadOffset = 0x13488;
    ...
    private const uint AlpineRidgeLoopExitAddress = 0x8008A320;
    ...
    private const int SourceCountRelativeOffset = 0x6AE0;
    private const int SourceTableRelativeOffset = 0x6AE4;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 198;
    private const int SourceCountAfter = 205;
    private const int KeyTrueIndex = 198;
    private const int LockedChestTrueIndex = 199;
    private const int KeyPropsRelativeOffset = 0xB160;
    private const int LockedChestPropsRelativeOffset = 0xB164;
    private const int RewardPropsRelativeOffset = 0xB17C;
    private const int ScenePointerFixupCountRelativeOffset = 0x116D8;
    private const int ScenePointerFixupListRelativeOffset = 0x116DC;
    private const int ScenePointerFixupCountBefore = 0xF6;
    private const int ScenePointerFixupCountAfter = 0xFD;

    private const int KeyRawX = 100000;
    private const int KeyRawY = 120000;
    private const int KeyRawZ = 23468;
    private const int LockedChestRawX = 103000;
    private const int LockedChestRawY = 120000;
    private const int LockedChestRawZ = 23468;

    private const int CopyBufferHiFileOffset = 0x4B5D0;
    private const int CopyBufferLoFileOffset = 0x4B5D4;
    private const int AlpineRidgeTreasureTableFileOffset = 0x5FC52;
    ...
    private const string ExpectedRetailOverlaySha256 = "f7c9a174f64959fb53b4058f9786df223a9c142b2289563ca31edc6a24c6d383";
    private const string ExpectedRetailDataEntrySha256 = "507c7b327bdfe9e4c230bef7d93da5d8c935a9a9f033ad99de22a5af8dfd0ddf";
    private const string ExpectedActorSubfileSha256 = "e2b9d049730e1724db21118b783294dd19e0460d03d1a9ad2536a05cec02b336";
    private const string ExpectedRetailSceneSha256 = "3dba83219b5e70e3a6c821ffc59b26643ca13be75d27581ebe9c421d8f4a46a6";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "40e69eb70ff4d7cdc44087044af03e20f0092de012f2cdfef833148fc3a6268a";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "925c6b52cac4088cb57e5ac6389047971971f7b50b4885fb6269eed98708876b";
    ...
    private static readonly int[] RewardMarkerTrueIndices = [200, 201, 202, 203, 204];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 200, 0xB17C, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 201, 0xB1A4, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 202, 0xB1CC, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 203, 0xB1F4, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 204, 0xB21C, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];

    private static readonly TextureRegion[] TextureRegions =
    [
        new("upper-lock", 0x000E, 192, 192, 0x4A2A, 0x000C, 0, 192, 0x26A0,
        ...
        new("lower-lock", 0x000E, 192, 224, 0x4C2A, 0x000C, 32, 192, 0x26A1,
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x000C, 64, 192, 0x26A2,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x000C, 96, 192, 0x26A3,
    ...
    public static AlpineRidgeNativeLockedChestCandidateResult Export(AlpineRidgeNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<AlpineRidgeNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            AlpineRidgeNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(31)}."
                    : "Disabled.",
                Verification: verification);

            return new AlpineRidgeNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, AlpineRidgeOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail AlpineRidge overlay");
        GuardSha256(ReadWad(source, layout, AlpineRidgeDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail AlpineRidge data entry");
        GuardSha256(ReadWad(source, layout, AlpineRidgeDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail AlpineRidge actor/model subfile");
        GuardSha256(ReadWad(source, layout, AlpineRidgeDataEntryOffset + AppendedLockedChestPackageOffset, SceneSize), ExpectedRetailSceneSha256, "retail Alpine Ridge scene");
        ...
        GuardSha256(ReadWad(source, layout, AlpineRidgeOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail AlpineRidge overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, DispatchHookAddress, AlpineRidgeLoopExitAddress, "retail Alpine Ridge class 0x00AE table slot");
        GuardWadWord(source, layout, DispatchHookAddress + 4, AlpineRidgeLoopExitAddress, "retail Alpine Ridge class 0x00AF neighbor table slot");
        GuardWadWord(source, layout, NativeKeyDispatchAddress, 0x80081654, "retail Alpine Ridge class 0x00AD Key table slot");
        GuardWadWord(source, layout, NativeRewardDispatchAddress, 0x8007BF5C, "retail Alpine Ridge class 0x000D reward table slot");
        GuardWadWord(source, layout, AlpineRidgeLoopExitAddress, 0x8FA90230, "retail Alpine Ridge actor-loop exit");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail AlpineRidge copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028009, "retail AlpineRidge copy-buffer HI16");
        GuardWord(copyPair, 4, 0x2442DEC0, "retail Alpine Ridge copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, AlpineRidgeTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Alpine Ridge treasure target is no longer the retail value 500.");
        }

        WadEntryLayout overlay = wad.ByIndex[AlpineRidgeOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[AlpineRidgeDataEntryIndex];
        if (overlay.Offset != AlpineRidgeOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != AlpineRidgeDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact AlpineRidge overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition level = catalog.FindByKey("alpineridge") ??
            throw new InvalidDataException("The level catalog does not contain Alpine Ridge.");
        if (level.SourceWadEntry != AlpineRidgeDataEntryIndex || level.LevelId != 31)
            throw new InvalidDataException("The Alpine Ridge level catalog mapping changed.");
        ...
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != AlpineRidgeDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"AlpineRidge texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private AlpineRidge texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private AlpineRidge texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
        if (selected.Count != 2176)
            throw new InvalidDataException($"Private Alpine Ridge texture allocation covers {selected.Count:N0} bytes, expected exactly 2,176.");
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = AlpineRidgeLoopExitAddress });
        ...
        if (lockedPayload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Alpine Ridge handler payload is 0x{lockedPayload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        return lockedPayload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, AlpineRidgeOverlayEntryOffset, OriginalOverlaySize);
        ...
        GuardWord(expanded, OverlayRelativeOffset(DispatchHookAddress) + 4, AlpineRidgeLoopExitAddress, "preserved Alpine Ridge class 0x00AF table slot");
        GuardWord(expanded, OverlayRelativeOffset(NativeKeyDispatchAddress), 0x80081654, "preserved Alpine Ridge Key table slot");
        GuardWord(expanded, OverlayRelativeOffset(NativeRewardDispatchAddress), 0x8007BF5C, "preserved Alpine Ridge reward table slot");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded AlpineRidge overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<AlpineRidgeNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, AlpineRidgeDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, SourceCountRelativeOffset + AppendedLockedChestPackageOffset, SourceCountBefore, "retail Alpine Ridge source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: AlpineRidgeDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: AlpineRidgeDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded AlpineRidge data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0x108, 4).ToArray(), "Alpine Ridge actor root slot 0x108");
        GuardBlank(expanded.AsSpan(0x1AC, 2).ToArray(), "Alpine Ridge actor id slot 0x1AC");
        WriteUInt32(expanded, 0x108, AppendedLockedChestPackageOffset);
        WriteUInt16(expanded, 0x1AC, 0x00AE);
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded AlpineRidge source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Alpine Ridge T198-T204 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "AlpineRidge Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "AlpineRidge Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"AlpineRidge T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail AlpineRidge scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "AlpineRidge appended scene pointer-fixup slots");
        ...
        List<AlpineRidgeNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new AlpineRidgeNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028009, "retail AlpineRidge copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x2442DEC0, "retail Alpine Ridge copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(AlpineRidgeTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Alpine Ridge treasure target is no longer 500.");
        WriteUInt16(bytes, AlpineRidgeTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                AlpineRidgeOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                AlpineRidgeDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[AlpineRidgeOverlayEntryIndex] != AlpineRidgeOverlayEntryOffset ||
            result[AlpineRidgeDataEntryIndex] != AlpineRidgeDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("AlpineRidge entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                AlpineRidgeOverlayEntryIndex => finalOverlay,
                AlpineRidgeDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The AlpineRidge candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                AlpineRidgeOverlayEntryIndex => ExpandedOverlaySize,
                AlpineRidgeDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[AlpineRidgeOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded AlpineRidge overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[AlpineRidgeDataEntryIndex], finalData.Length), finalData, "expanded AlpineRidge data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[AlpineRidgeOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[AlpineRidgeDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not AlpineRidgeOverlayEntryIndex and not AlpineRidgeDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[AlpineRidgeDataEntryIndex],
            ...
            actorLayout.NativeRootCount != 47 || actorLayout.LastNativeRoot != AppendedLockedChestPackageOffset)
        {
            throw new InvalidDataException($"Expanded AlpineRidge actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028009, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x2442E600, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(AlpineRidgeTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Alpine Ridge treasure target did not read back as 510.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final AlpineRidge source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final AlpineRidge pointer-fixup count");
        GuardUInt32(finalData, 0x108, AppendedLockedChestPackageOffset, "final Alpine Ridge 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x1AC, 2)) != 0x00AE)
            throw new InvalidDataException("Final Alpine Ridge actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; 0x800-byte overlay plus 0x1800-byte actor/model WAD growth; copy buffer moved from 0x8008DEC0 to 0x8008E600 immediately after the transplanted handler; SCUS relocation 53875->53879; direct guarded class-0x00AE table entry with Alpine Ridge-resident 0x00AD/0x000D/debris dependencies; T198-T204 source rows/props/fixups; four exact zero-preimage, ownership-proven private texture/CLUT regions; treasure 500->510; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final-image readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, AlpineRidgeOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(AlpineRidgeNativeLockedChestCandidateRequest request)
    {
