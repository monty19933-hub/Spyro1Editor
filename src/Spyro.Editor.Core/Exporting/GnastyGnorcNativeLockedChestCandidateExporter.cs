...
public sealed record GnastyGnorcNativeLockedChestCandidateRequest(
...
public sealed record GnastyGnorcNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record GnastyGnorcNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<GnastyGnorcNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record GnastyGnorcNativeLockedChestCandidateResult(
    string OutputImagePath,
    GnastyGnorcNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Gnasty Gnorc transplant of Peace Keepers' native Key + Locked Chest
/// source bundle. Gnasty Gnorc already owns the retail Key dispatch, class 0x000D
...
/// </summary>
public static class GnastyGnorcNativeLockedChestCandidateExporter
{
    public const string RecipeId = "gnastygnorc.peacekeepers.nativeLockedChest.disposable.v1";
    ...
    private const int GnastyGnorcOverlayEntryIndex = 75;
    private const long GnastyGnorcOverlayEntryOffset = 0x5911800;
    private const int OriginalOverlaySize = 0xB000;
    private const int ExpandedOverlaySize = 0xB800;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x800854B4;
    private const uint RelocatedCopyBufferAddress = 0x80085C10;

    private const int GnastyGnorcDataEntryIndex = 76;
    private const long GnastyGnorcDataEntryOffset = 0x591C800;
    private const int OriginalDataEntrySize = 0x1CE000;
    private const int ExpandedDataEntrySize = 0x1CF800;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x167000;
    private const int OriginalActorSubfileSize = 0x46800;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1AD800;
    private const int ExpandedSceneOffset = 0x1AF000;
    private const int SceneSize = 0x20800;
    ...
    private const int DispatchShimLength = 0x14;
    private const int DispatchPaddingLength = 0x08;
    private const int OverlayPayloadOffset = 0xAA7C;
    private const int OverlayPayloadLength = LockedHandlerLength + DispatchShimLength + DispatchPaddingLength;
    private const uint DispatchHookAddress = 0x8007AF9C;
    private const uint NativeKeyDispatchAddress = 0x8007AE3C;
    private const uint NativeRewardDispatchAddress = 0x8007AE7C;
    private const uint GnastyGnorcLoopExitAddress = 0x80082ED8;
    ...
    private const int SourceCountRelativeOffset = 0x13104;
    private const int SourceTableRelativeOffset = 0x13108;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 113;
    private const int SourceCountAfter = 120;
    private const int KeyTrueIndex = 113;
    private const int LockedChestTrueIndex = 114;
    private const int KeyPropsRelativeOffset = 0x15A50;
    private const int LockedChestPropsRelativeOffset = 0x15A54;
    private const int RewardPropsRelativeOffset = 0x15A6C;
    private const int ScenePointerFixupCountRelativeOffset = 0x1FFF4;
    private const int ScenePointerFixupListRelativeOffset = 0x1FFF8;
    private const int ScenePointerFixupCountBefore = 119;
    private const int ScenePointerFixupCountAfter = 126;

    private const int KeyRawX = 83500;
    private const int KeyRawY = 45500;
    private const int KeyRawZ = 10614;
    private const int LockedChestRawX = 85500;
    private const int LockedChestRawY = 45500;
    private const int LockedChestRawZ = 10614;

    private const int CopyBufferHiFileOffset = 0x4BDF0;
    private const int CopyBufferLoFileOffset = 0x4BDF4;
    private const int TreasureTableFileOffset = 0x5FC38;
    private const int TreasureTableLength = 0x48;
    private const int GnastyGnorcTreasureTableIndex = 33;
    private const int GnastyGnorcTreasureTableFileOffset = 0x5FC7A;
    ...
    private const string ExpectedRetailOverlaySha256 = "9b8b9677df191c917ed45896192db86c82cdcad2853ba1cd7dfd6feca3be43a4";
    private const string ExpectedRetailDataEntrySha256 = "c7252c1d79657ab0c3c7aa847ef3dfa1694589479b1bf6b2370a051ff0bdcc69";
    private const string ExpectedActorSubfileSha256 = "66aa5b0c68944ff3150320ec07982c335030ab8c8d8018179b47428a46950f4b";
    private const string ExpectedRetailSceneSha256 = "8ecb13c6690091018f39cbe1ce58f9d7e0a5438ceb96354acc7900d2ce2acc9e";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "be9fdff2cf1ec59c563cba9af17315d1da09a182b52e94f846f7f161ce52f553";
    ...
    private const string ExpectedHandlerPayloadSha256 = "6a193c09e8b44789a26db2131d7f6c0aadaf29b53a7c2b9528812164ed223d6f";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "2c5101930f1baf0b9572f4cfa6d1fb862f1d60b90ddf511033d3b44b461cdb28";
    private const string ExpectedTreasureTableSha256 = "641867a8c3d0c8af833f4ce678cd27ff7520e7ddafa4cecfd19c7a388f625ee6";
    ...
    private static readonly int[] RewardMarkerTrueIndices = [115, 116, 117, 118, 119];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 115, 0x15A6C, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 116, 0x15A94, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 117, 0x15ABC, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 118, 0x15AE4, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 119, 0x15B0C, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];

    private static readonly TextureRegion[] TextureRegions =
    [
        new("upper-lock", 0x000E, 192, 192, 0x4A2A, 0x0008, 0, 64, 0x0EE0,
        ...
        new("lower-lock", 0x000E, 192, 224, 0x4C2A, 0x0008, 32, 64, 0x0EE1,
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x0008, 64, 64, 0x0EE2,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x0008, 96, 64, 0x0EE3,
    ...
    public static GnastyGnorcNativeLockedChestCandidateResult Export(GnastyGnorcNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<GnastyGnorcNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            GnastyGnorcNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(63)}."
                    : "Disabled.",
                Verification: verification);

            return new GnastyGnorcNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, GnastyGnorcOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail GnastyGnorc overlay");
        GuardSha256(ReadWad(source, layout, GnastyGnorcDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail GnastyGnorc data entry");
        GuardSha256(ReadWad(source, layout, GnastyGnorcDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail GnastyGnorc actor/model subfile");
        GuardSha256(ReadWad(source, layout, GnastyGnorcDataEntryOffset + AppendedLockedChestPackageOffset, SceneSize), ExpectedRetailSceneSha256, "retail Gnasty Gnorc scene");
        ...
        GuardSha256(ReadWad(source, layout, GnastyGnorcOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail GnastyGnorc overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, DispatchHookAddress, 0x08020BB6, "retail Gnasty Gnorc class 0xAE default jump");
        GuardWadWord(source, layout, DispatchHookAddress + 4, 0, "retail Gnasty Gnorc class 0xAE delay word");
        GuardWadWord(source, layout, NativeKeyDispatchAddress - 4, 0x240200AD, "retail Gnasty Gnorc Key compare");
        GuardWadWord(source, layout, NativeKeyDispatchAddress, 0x10621537, "retail Gnasty Gnorc Key branch");
        GuardWadWord(source, layout, NativeRewardDispatchAddress - 4, 0x2402000D, "retail Gnasty Gnorc reward compare");
        GuardWadWord(source, layout, NativeRewardDispatchAddress, 0x1062007B, "retail Gnasty Gnorc reward branch");
        GuardWadWord(source, layout, GnastyGnorcLoopExitAddress, 0x8FA90210, "retail Gnasty Gnorc actor-loop exit");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail GnastyGnorc copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028008, "retail GnastyGnorc copy-buffer HI16");
        GuardWord(copyPair, 4, 0x244254B4, "retail Gnasty Gnorc copy-buffer LO16");
        if (GnastyGnorcTreasureTableFileOffset != TreasureTableFileOffset + (GnastyGnorcTreasureTableIndex * 2))
            throw new InvalidDataException("The independently indexed Gnasty Gnorc treasure slot no longer resolves to 0x5FC7A.");
        GuardSha256(
            DiscImage.ReadFileBytes(source, layout, executable.Lba, TreasureTableFileOffset, TreasureTableLength),
            ExpectedTreasureTableSha256,
            "retail 35-level treasure table plus terminator");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, GnastyGnorcTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The independently indexed Gnasty Gnorc treasure target is no longer the retail value 500.");
        }

        WadEntryLayout overlay = wad.ByIndex[GnastyGnorcOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[GnastyGnorcDataEntryIndex];
        if (overlay.Offset != GnastyGnorcOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != GnastyGnorcDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact GnastyGnorc overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition level = catalog.FindByKey("gnastygnorc") ??
            throw new InvalidDataException("The level catalog does not contain Gnasty Gnorc.");
        if (level.SourceWadEntry != GnastyGnorcDataEntryIndex || level.LevelId != 63)
            throw new InvalidDataException("The Gnasty Gnorc level catalog mapping changed.");
        ...
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != GnastyGnorcDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"GnastyGnorc texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private GnastyGnorc texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private GnastyGnorc texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
        if (selected.Count != 2176)
            throw new InvalidDataException($"Private Gnasty Gnorc texture allocation covers {selected.Count:N0} bytes, expected exactly 2,176.");
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = GnastyGnorcLoopExitAddress });
        ...
        byte[] dispatch = Words(
            ...
            0,
            EncodeJump(GnastyGnorcLoopExitAddress),
            0);
        if (dispatch.Length != DispatchShimLength)
            throw new InvalidDataException("Gnasty Gnorc Locked Chest dispatch shim length changed.");

        byte[] payload = Concat(lockedPayload, dispatch, new byte[DispatchPaddingLength]);
        if (payload.Length != OverlayPayloadLength || LockedHandlerAddress + payload.Length != RelocatedCopyBufferAddress)
            throw new InvalidDataException($"Gnasty Gnorc handler/alignment payload is 0x{payload.Length:X}, expected 0x{OverlayPayloadLength:X} ending at 0x{RelocatedCopyBufferAddress:X8}.");
        GuardSha256(payload, ExpectedHandlerPayloadSha256, "Gnasty Gnorc handler/shim/alignment payload");
        return payload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, GnastyGnorcOverlayEntryOffset, OriginalOverlaySize);
        ...
        GuardWord(expanded, OverlayRelativeOffset(NativeKeyDispatchAddress), 0x10621537, "preserved Gnasty Gnorc Key branch");
        GuardWord(expanded, OverlayRelativeOffset(NativeRewardDispatchAddress), 0x1062007B, "preserved Gnasty Gnorc reward branch");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded GnastyGnorc overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<GnastyGnorcNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, GnastyGnorcDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, SourceCountRelativeOffset + AppendedLockedChestPackageOffset, SourceCountBefore, "retail Gnasty Gnorc source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: GnastyGnorcDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: GnastyGnorcDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded GnastyGnorc data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0xD0, 4).ToArray(), "Gnasty Gnorc actor root slot 0xD0");
        GuardBlank(expanded.AsSpan(0x190, 2).ToArray(), "Gnasty Gnorc actor id slot 0x190");
        WriteUInt32(expanded, 0xD0, AppendedLockedChestPackageOffset);
        WriteUInt16(expanded, 0x190, 0x00AE);
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded GnastyGnorc source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Gnasty Gnorc T113-T119 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "GnastyGnorc Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "GnastyGnorc Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"GnastyGnorc T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail GnastyGnorc scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "GnastyGnorc appended scene pointer-fixup slots");
        ...
        List<GnastyGnorcNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new GnastyGnorcNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028008, "retail GnastyGnorc copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x244254B4, "retail Gnasty Gnorc copy-buffer LO16");
        ...
        GuardSha256(bytes.AsSpan(TreasureTableFileOffset, TreasureTableLength).ToArray(), ExpectedTreasureTableSha256,
            "retail 35-level treasure table plus terminator before exact-slot patch");
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(GnastyGnorcTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Gnasty Gnorc treasure target is no longer 500.");
        WriteUInt16(bytes, GnastyGnorcTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                GnastyGnorcOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                GnastyGnorcDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[GnastyGnorcOverlayEntryIndex] != GnastyGnorcOverlayEntryOffset ||
            result[GnastyGnorcDataEntryIndex] != GnastyGnorcDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("GnastyGnorc entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                GnastyGnorcOverlayEntryIndex => finalOverlay,
                GnastyGnorcDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The GnastyGnorc candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                GnastyGnorcOverlayEntryIndex => ExpandedOverlaySize,
                GnastyGnorcDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[GnastyGnorcOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded GnastyGnorc overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[GnastyGnorcDataEntryIndex], finalData.Length), finalData, "expanded GnastyGnorc data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[GnastyGnorcOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[GnastyGnorcDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not GnastyGnorcOverlayEntryIndex and not GnastyGnorcDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[GnastyGnorcDataEntryIndex],
            ...
            actorLayout.NativeRootCount != 33 || actorLayout.LastNativeRoot != AppendedLockedChestPackageOffset)
        {
            throw new InvalidDataException($"Expanded GnastyGnorc actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028008, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x24425C10, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(GnastyGnorcTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Gnasty Gnorc treasure target did not read back as 510.");
        DiscFileRecord sourceExecutable = DiscImage.FindRootFileRecord(source, layout, IsExecutableName);
        byte[] expectedTreasureTable = DiscImage.ReadFileBytes(
            source, layout, sourceExecutable.Lba, TreasureTableFileOffset, TreasureTableLength);
        GuardSha256(expectedTreasureTable, ExpectedTreasureTableSha256, "retail treasure-table verification source");
        WriteUInt16(expectedTreasureTable, GnastyGnorcTreasureTableFileOffset - TreasureTableFileOffset, TreasureTargetAfter);
        GuardEqual(finalExecutable.AsSpan(TreasureTableFileOffset, TreasureTableLength).ToArray(), expectedTreasureTable,
            "final treasure table with only Gnasty Gnorc slot changed");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final GnastyGnorc source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final GnastyGnorc pointer-fixup count");
        GuardUInt32(finalData, 0xD0, AppendedLockedChestPackageOffset, "final Gnasty Gnorc 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x190, 2)) != 0x00AE)
            throw new InvalidDataException("Final Gnasty Gnorc actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; one 0x2000-byte WAD growth (0x800 overlay + 0x1800 actor/model); aligned copy buffer moved to 0x80085C10; SCUS relocation 53875->53879; native Peace Keepers 0x00AE handler/model with Gnasty Gnorc-resident 0x00AD/0x000D/debris dependencies; T113-T119 source rows/props/fixups; four ownership-proven private texture/CLUT regions; independently indexed treasure slot 33 changed 500->510 with every neighboring table word preserved; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, GnastyGnorcOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(GnastyGnorcNativeLockedChestCandidateRequest request)
    {
