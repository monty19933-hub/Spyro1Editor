...
public sealed record DarkPassageNativeLockedChestCandidateRequest(
...
public sealed record DarkPassageNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record DarkPassageNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<DarkPassageNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record DarkPassageNativeLockedChestCandidateResult(
    string OutputImagePath,
    DarkPassageNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Dark Passage transplant of Peace Keepers' native Key + Locked Chest
/// source bundle. Dark Passage already owns the retail Key dispatch, class 0x000D
...
/// </summary>
public static class DarkPassageNativeLockedChestCandidateExporter
{
    public const string RecipeId = "darkpassage.peacekeepers.nativeLockedChest.disposable.v1";
    ...
    private const int DarkPassageOverlayEntryIndex = 59;
    private const long DarkPassageOverlayEntryOffset = 0x467A000;
    private const int OriginalOverlaySize = 0xD800;
    private const int ExpandedOverlaySize = 0xE000;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x800880D4;
    private const uint RelocatedCopyBufferAddress = 0x80088820;

    private const int DarkPassageDataEntryIndex = 60;
    private const long DarkPassageDataEntryOffset = 0x4687800;
    private const int OriginalDataEntrySize = 0x2B4800;
    private const int ExpandedDataEntrySize = 0x2B6000;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x179800;
    private const int OriginalActorSubfileSize = 0x54000;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1CD800;
    private const int ExpandedSceneOffset = 0x1CF000;
    private const int SceneSize = 0x10000;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const int OverlayPayloadOffset = 0xD69C;
    ...
    private const uint DarkPassageLoopExitAddress = 0x80084B48;
    ...
    private const int SourceCountRelativeOffset = 0x459C;
    private const int SourceTableRelativeOffset = 0x45A0;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 230;
    private const int SourceCountAfter = 237;
    private const int KeyTrueIndex = 230;
    private const int LockedChestTrueIndex = 231;
    private const int KeyPropsRelativeOffset = 0x9720;
    private const int LockedChestPropsRelativeOffset = 0x9724;
    private const int RewardPropsRelativeOffset = 0x973C;
    private const int ScenePointerFixupCountRelativeOffset = 0xF800;
    private const int ScenePointerFixupListRelativeOffset = 0xF804;
    private const int ScenePointerFixupCountBefore = 261;
    private const int ScenePointerFixupCountAfter = 268;

    private const int KeyRawX = 76000;
    private const int KeyRawY = 41933;
    private const int KeyRawZ = 38920;
    private const int LockedChestRawX = 73500;
    private const int LockedChestRawY = 41933;
    private const int LockedChestRawZ = 38920;

    private const int CopyBufferHiFileOffset = 0x4BAD0;
    private const int CopyBufferLoFileOffset = 0x4BAD4;
    private const int DarkPassageTreasureTableFileOffset = 0x5FC6A;
    private const int NeighborTreasureTableFileOffset = 0x5FC6C;
    private const ushort NeighborTreasureTarget = 400;
    ...
    private const string ExpectedRetailOverlaySha256 = "a8b99270c37a37363824cf1fd2081c092968abef62df9e74d70ae0017b984727";
    private const string ExpectedRetailDataEntrySha256 = "217d4e525f10b63f4ae4a413d807989241b7f2f91963b543b421cbd3954c364a";
    private const string ExpectedActorSubfileSha256 = "4ffa23a02961a90f0df04b215b83602e91fe30878ee68fa3e4ab13b070300031";
    private const string ExpectedRetailSceneSha256 = "adf35561dd375532982797b131b16af96145594d2e739f8f3be3efa819bcc8d8";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "a9c0d179b44788454fc04c549b9fdecdf3a5280214c763b97d22c08adda1c83f";
    ...
    private const string ExpectedHandlerPayloadSha256 = "1e47ce98002edff73b9f3fde2ab2d164184aac1c93049d4b70355ba09ecf245a";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "15c32ea660d457f7cc3fbe2012b9df252a8fbd107a5177831acb77e3c4eff221";
    ...
    private static readonly int[] RewardMarkerTrueIndices = [232, 233, 234, 235, 236];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 232, 0x973C, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 233, 0x9764, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 234, 0x978C, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 235, 0x97B4, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 236, 0x97DC, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];

    private static readonly TextureRegion[] TextureRegions =
    [
        new("upper-lock", 0x000E, 192, 192, 0x4A2A, 0x000A, 0, 64, 0x102A,
        ...
        new("lower-lock", 0x000E, 192, 224, 0x4C2A, 0x000A, 32, 64, 0x102B,
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x000A, 64, 64, 0x102C,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x000A, 96, 64, 0x102D,
    ...
    public static DarkPassageNativeLockedChestCandidateResult Export(DarkPassageNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<DarkPassageNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            DarkPassageNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(51)}."
                    : "Disabled.",
                Verification: verification);

            return new DarkPassageNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, DarkPassageOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail DarkPassage overlay");
        GuardSha256(ReadWad(source, layout, DarkPassageDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail DarkPassage data entry");
        GuardSha256(ReadWad(source, layout, DarkPassageDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail DarkPassage actor/model subfile");
        GuardSha256(ReadWad(source, layout, DarkPassageDataEntryOffset + AppendedLockedChestPackageOffset, SceneSize), ExpectedRetailSceneSha256, "retail Dark Passage scene");
        ...
        GuardSha256(ReadWad(source, layout, DarkPassageOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail DarkPassage overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, DispatchHookAddress, DarkPassageLoopExitAddress, "retail Dark Passage class 0x00AE table slot");
        GuardWadWord(source, layout, DispatchHookAddress + 4, DarkPassageLoopExitAddress, "retail Dark Passage class 0x00AF neighbor table slot");
        GuardWadWord(source, layout, NativeKeyDispatchAddress, 0x80081294, "retail Dark Passage class 0x00AD Key table slot");
        GuardWadWord(source, layout, NativeRewardDispatchAddress, 0x8007B8F4, "retail Dark Passage class 0x000D reward table slot");
        GuardWadWord(source, layout, DarkPassageLoopExitAddress, 0x8FA90280, "retail Dark Passage actor-loop exit");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail DarkPassage copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028009, "retail DarkPassage copy-buffer HI16");
        GuardWord(copyPair, 4, 0x244280D4, "retail Dark Passage copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, DarkPassageTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Dark Passage treasure target is no longer the retail value 500.");
        }
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, NeighborTreasureTableFileOffset, 2)) != NeighborTreasureTarget)
        {
            throw new InvalidDataException("The adjacent Lofty Castle treasure target is no longer the retail value 400.");
        }

        WadEntryLayout overlay = wad.ByIndex[DarkPassageOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[DarkPassageDataEntryIndex];
        if (overlay.Offset != DarkPassageOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != DarkPassageDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact DarkPassage overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition level = catalog.FindByKey("darkpassage") ??
            throw new InvalidDataException("The level catalog does not contain Dark Passage.");
        if (level.SourceWadEntry != DarkPassageDataEntryIndex || level.LevelId != 51)
            throw new InvalidDataException("The Dark Passage level catalog mapping changed.");
        ...
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != DarkPassageDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"DarkPassage texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private DarkPassage texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private DarkPassage texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
        if (selected.Count != 2176)
            throw new InvalidDataException($"Private Dark Passage texture allocation covers {selected.Count:N0} bytes, expected exactly 2,176.");
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = DarkPassageLoopExitAddress });
        ...
        if (lockedPayload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Dark Passage handler payload is 0x{lockedPayload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        GuardSha256(lockedPayload, ExpectedHandlerPayloadSha256, "relocated Dark Passage Locked Chest handler payload");
        return lockedPayload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, DarkPassageOverlayEntryOffset, OriginalOverlaySize);
        ...
        GuardWord(expanded, OverlayRelativeOffset(DispatchHookAddress) + 4, DarkPassageLoopExitAddress, "preserved Dark Passage class 0x00AF table slot");
        GuardWord(expanded, OverlayRelativeOffset(NativeKeyDispatchAddress), 0x80081294, "preserved Dark Passage Key table slot");
        GuardWord(expanded, OverlayRelativeOffset(NativeRewardDispatchAddress), 0x8007B8F4, "preserved Dark Passage reward table slot");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded DarkPassage overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<DarkPassageNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, DarkPassageDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, SourceCountRelativeOffset + AppendedLockedChestPackageOffset, SourceCountBefore, "retail Dark Passage source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: DarkPassageDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: DarkPassageDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded DarkPassage data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0x110, 4).ToArray(), "Dark Passage actor root slot 0x110");
        GuardBlank(expanded.AsSpan(0x1B0, 2).ToArray(), "Dark Passage actor id slot 0x1B0");
        WriteUInt32(expanded, 0x110, AppendedLockedChestPackageOffset);
        WriteUInt16(expanded, 0x1B0, 0x00AE);
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded DarkPassage source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Dark Passage T230-T236 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "DarkPassage Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "DarkPassage Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"DarkPassage T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail DarkPassage scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "DarkPassage appended scene pointer-fixup slots");
        ...
        List<DarkPassageNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new DarkPassageNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028009, "retail DarkPassage copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x244280D4, "retail Dark Passage copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(DarkPassageTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Dark Passage treasure target is no longer 500.");
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(NeighborTreasureTableFileOffset, 2)) != NeighborTreasureTarget)
            throw new InvalidDataException("The adjacent Lofty Castle treasure target is no longer 400.");
        WriteUInt16(bytes, DarkPassageTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                DarkPassageOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                DarkPassageDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[DarkPassageOverlayEntryIndex] != DarkPassageOverlayEntryOffset ||
            result[DarkPassageDataEntryIndex] != DarkPassageDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("DarkPassage entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                DarkPassageOverlayEntryIndex => finalOverlay,
                DarkPassageDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The DarkPassage candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                DarkPassageOverlayEntryIndex => ExpandedOverlaySize,
                DarkPassageDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[DarkPassageOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded DarkPassage overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[DarkPassageDataEntryIndex], finalData.Length), finalData, "expanded DarkPassage data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[DarkPassageOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[DarkPassageDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not DarkPassageOverlayEntryIndex and not DarkPassageDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[DarkPassageDataEntryIndex],
            ...
            actorLayout.NativeRootCount != 49 || actorLayout.LastNativeRoot != AppendedLockedChestPackageOffset)
        {
            throw new InvalidDataException($"Expanded DarkPassage actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028009, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x24428820, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(DarkPassageTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Dark Passage treasure target did not read back as 510.");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(NeighborTreasureTableFileOffset, 2)) != NeighborTreasureTarget)
            throw new InvalidDataException("The adjacent Lofty Castle treasure target changed during Dark Passage export.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final DarkPassage source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final DarkPassage pointer-fixup count");
        GuardUInt32(finalData, 0x110, AppendedLockedChestPackageOffset, "final Dark Passage 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x1B0, 2)) != 0x00AE)
            throw new InvalidDataException("Final Dark Passage actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; 0x800-byte overlay plus 0x1800-byte actor/model WAD growth; copy buffer moved from 0x800880D4 to aligned 0x80088820 after the transplanted handler; SCUS relocation 53875->53879; direct guarded class-0x00AE table entry with Dark Passage-resident 0x00AD/0x000D/debris dependencies; T230-T236 source rows/props/fixups; four exact zero-preimage, ownership-proven private texture/CLUT regions; independently verified Dark Passage treasure target 500->510 at SCUS 0x5FC6A while adjacent 0x5FC6C remains 400; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final-image readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, DarkPassageOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(DarkPassageNativeLockedChestCandidateRequest request)
    {
