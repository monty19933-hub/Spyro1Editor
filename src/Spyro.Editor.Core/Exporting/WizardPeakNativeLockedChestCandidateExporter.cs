...
public sealed record WizardPeakNativeLockedChestCandidateRequest(
...
public sealed record WizardPeakNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record WizardPeakNativeLockedChestCandidatePlan(
    ...
    int SceneFixupTailInsertionBytes,
    string ShiftedSceneSuffixSha256,
    ...
    IReadOnlyList<WizardPeakNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record WizardPeakNativeLockedChestCandidateResult(
    string OutputImagePath,
    WizardPeakNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Wizard Peak transplant of Peace Keepers' native Key + Locked
/// Chest source bundle. Wizard Peak owns the native Key and metal-debris
/// dependencies, but lacks both class 0x00AE and the class 0x000D one-shot reward
/// handler. This guarded candidate moves the two handlers and their exact shared
/// branches as one 0xBC0-byte overlay bundle, imports one locked-chest package,
/// and allocates four source-bound private texture/CLUT regions. It intentionally
/// remains outside the editor registry until DuckStation runtime proof exists.
/// </summary>
public static class WizardPeakNativeLockedChestCandidateExporter
{
    public const string RecipeId = "wizardpeak.peacekeepers.nativeLockedChest.rewardClosure.disposable.v1";

    private const int SectorBytes = 2048;
    private const int WadLba = 37;
    private const int OriginalWadSize = 0x6927000;
    private const int ExpandedWadSize = 0x6929800;
    private const int OriginalExecutableLba = 53875;
    private const int RelocatedExecutableLba = 53880;
    private const int ExecutableSize = 0x66000;
    private const int NextFileLba = 60000;

    private const int WizardPeakOverlayEntryIndex = 39;
    private const long WizardPeakOverlayEntryOffset = 0x2F5B000;
    private const int OriginalOverlaySize = 0x10000;
    private const int ExpandedOverlaySize = 0x11000;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x8008A8A0;
    private const uint RelocatedCopyBufferAddress = 0x8008BA38;

    private const int WizardPeakDataEntryIndex = 40;
    private const long WizardPeakDataEntryOffset = 0x2F6B000;
    private const int OriginalDataEntrySize = 0x28A000;
    private const int ExpandedDataEntrySize = 0x28B800;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x174000;
    private const int OriginalActorSubfileSize = 0x5A800;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1CE800;
    private const int ExpandedSceneOffset = 0x1D0000;
    private const int SceneSize = 0x11800;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const uint RewardHandlerAddress = 0x8008AFE0;
    private const int RewardHandlerLength = 0x438;
    private const uint RewardDispatchAddress = 0x8008B418;
    private const uint LockedDispatchAddress = 0x8008B42C;
    private const uint DeleteThunkAddress = 0x8008B440;
    private const uint SoundThunkAddress = 0x8008B450;
    private const int OverlayPayloadOffset = 0xFE68;
    private const int OverlayPayloadLength = 0xBC0;
    private const uint RewardDispatchHookAddress = 0x8007AA4C;
    private const uint LockedDispatchHookAddress = 0x8007ACD0;
    private const uint NativeKeyDispatchAddress = 0x8007ACCC;
    private const uint WizardPeakLoopExitAddress = 0x80086D8C;
    private const uint WizardPeakResetAddress = 0x80085A4C;
    ...
    private const long RewardHandlerWadOffset = 0x1881B64;
    private const uint RewardHandlerSourceAddress = 0x8007E59C;
    private const int RewardHandlerSourceLength = 0x434;
    ...
    private const int SourceCountRelativeOffset = 0x6174;
    private const int SourceTableRelativeOffset = 0x6178;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 165;
    private const int SourceCountAfter = 172;
    private const int KeyTrueIndex = 165;
    private const int LockedChestTrueIndex = 166;
    private const int KeyPropsRelativeOffset = 0x9C98;
    private const int LockedChestPropsRelativeOffset = 0x9C9C;
    private const int RewardPropsRelativeOffset = 0x9CB4;
    private const int ScenePointerFixupCountRelativeOffset = 0xFC3C;
    private const int ScenePointerFixupListRelativeOffset = 0xFC40;
    private const int ScenePointerFixupCountBefore = 191;
    private const int ScenePointerFixupCountAfter = 198;
    private const int SceneFixupTailInsertRelativeOffset = 0xFF3C;
    private const int SceneFixupTailInsertionBytes = 0x1C;
    private const int ShiftedSceneSuffixLength = 0xFF0;
    private const int SceneZeroTailBefore = 0x8D4;
    private const int SceneZeroTailAfter = 0x8B8;

    private const int KeyRawX = 132000;
    private const int KeyRawY = 100000;
    private const int KeyRawZ = 22527;
    private const int LockedChestRawX = 135000;
    private const int LockedChestRawY = 100000;
    private const int LockedChestRawZ = 22527;

    private const int CopyBufferHiFileOffset = 0x4B670;
    private const int CopyBufferLoFileOffset = 0x4B674;
    private const int WizardPeakTreasureTableFileOffset = 0x5FC56;
    ...
    private const string ExpectedRetailOverlaySha256 = "5d3996111567ffac6eca08d340f2f691824bb7969ba2a0cd95e9346ccfe157e1";
    private const string ExpectedRetailDataEntrySha256 = "9973493f865b5e86bc17b32c5af9e3d7596e25615618c007ed28e0379161f2d8";
    private const string ExpectedActorSubfileSha256 = "254e4de3a4537a65d65c53774701fc8591e9eea3359057e07a0837b526d8962f";
    private const string ExpectedRetailSceneSha256 = "1e204311534f03f09804d55ca833c62a1a3cc3665dc21d88de743d820103a951";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "e809be82fcbd8f36dfaca130b139020faa59d34f173e00cde3cc37de1eac3f71";
    ...
    private const string ExpectedRewardHandlerSourceSha256 = "129d8511c63925af80ab73a02def446efc39e23974255fd4b8d38f51411c2056";
    private const string ExpectedHandlerPayloadSha256 = "c26a6a53c55e2fccc66a0f869904bb573f401a0708084d5663c7db9167fdfc73";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "ace3900d18b6515c090396f1ec1f800e10967a5b5a77f096ba1eba915d1cd02e";
    private const string ExpectedRowPropertyPreimageSha256 = "c38ddce4c7d430b93408979c091f901ac3e5cbb112fdef114e87b683b09ef8ff";
    private const string ExpectedShiftedSceneSuffixSha256 = "2e13770149961880250628cd95711483a4b955b30e8847846539ea7cda55d438";
    ...
    private static readonly int[] RewardMarkerTrueIndices = [167, 168, 169, 170, 171];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 167, 0x9CB4, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 168, 0x9CDC, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 169, 0x9D04, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 170, 0x9D2C, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 171, 0x9D54, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];

    private static readonly TextureRegion[] TextureRegions =
    [
        new("upper-lock", 0x000E, 192, 192, 0x4A2A, 0x001E, 64, 0, 0x1120,
        ...
        new("lower-lock", 0x000E, 192, 224, 0x4C2A, 0x001E, 96, 0, 0x1121,
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x001E, 128, 0, 0x1122,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x001E, 160, 0, 0x1123,
    ...
    public static WizardPeakNativeLockedChestCandidateResult Export(WizardPeakNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<WizardPeakNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            WizardPeakNativeLockedChestCandidatePlan plan = new(
                ...
                SceneFixupTailInsertionBytes: SceneFixupTailInsertionBytes,
                ShiftedSceneSuffixSha256: ExpectedShiftedSceneSuffixSha256,
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(33)}."
                    : "Disabled.",
                Verification: verification);

            return new WizardPeakNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, WizardPeakOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail WizardPeak overlay");
        GuardSha256(ReadWad(source, layout, WizardPeakDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail WizardPeak data entry");
        GuardSha256(ReadWad(source, layout, WizardPeakDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail WizardPeak actor/model subfile");
        GuardSha256(ReadWad(source, layout, WizardPeakDataEntryOffset + AppendedLockedChestPackageOffset, SceneSize), ExpectedRetailSceneSha256, "retail WizardPeak scene");
        ...
        GuardSha256(ReadWad(source, layout, WizardPeakOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail WizardPeak overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, RewardDispatchHookAddress, WizardPeakLoopExitAddress, "retail Wizard Peak class 0x000D dispatch pointer");
        GuardWadWord(source, layout, LockedDispatchHookAddress, WizardPeakLoopExitAddress, "retail Wizard Peak class 0x00AE dispatch pointer");
        GuardWadWord(source, layout, NativeKeyDispatchAddress, 0x80081AB0, "retail Wizard Peak class 0x00AD dispatch pointer");
        GuardSha256(
            ReadWad(source, layout, WizardPeakOverlayEntryOffset + (WizardPeakResetAddress - OverlayLoadAddress) - 0x10, 0x40),
            "23aabbf138f06867c52f9a0c903236b5acf57bc39db7764c174b7d3d97c35dff",
            "retail Wizard Peak reset continuation window");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail WizardPeak copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028009, "retail Wizard Peak copy-buffer HI16");
        GuardWord(copyPair, 4, 0x2442A8A0, "retail Wizard Peak copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, WizardPeakTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Wizard Peak treasure target is no longer the retail value 500.");
        }

        WadEntryLayout overlay = wad.ByIndex[WizardPeakOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[WizardPeakDataEntryIndex];
        if (overlay.Offset != WizardPeakOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != WizardPeakDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact WizardPeak overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition wizardPeak = catalog.FindByKey("wizardpeak") ??
            throw new InvalidDataException("The level catalog does not contain WizardPeak.");
        if (wizardPeak.SourceWadEntry != WizardPeakDataEntryIndex || wizardPeak.LevelId != 33)
            throw new InvalidDataException("The WizardPeak level catalog mapping changed.");

        NativeTexturePageOwnershipReport report = NativeTexturePageOwnershipScanner.Scan(sourcePath, wizardPeak);
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != WizardPeakDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"WizardPeak texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private WizardPeak texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private WizardPeak texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
        if (selected.Count != 2176)
            throw new InvalidDataException($"Private WizardPeak texture allocation covers {selected.Count:N0} bytes, expected exactly 2,176.");
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] reward = ReadWad(retail, layout, RewardHandlerWadOffset, RewardHandlerSourceLength);
        ...
        GuardSha256(reward, ExpectedRewardHandlerSourceSha256, "Peace Keepers class 0x000D reward handler");
        ...
        byte[] relocatedLockedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = WizardPeakLoopExitAddress });
        ...
        CodeSegment[] rewardSegments =
        [
            new(RewardHandlerSourceAddress, RewardHandlerAddress + 4, reward)
        ];
        byte[] relocatedRewardBody = RelocateCode(
            rewardSegments,
            new Dictionary<uint, uint>
            {
                [0x8008A20C] = WizardPeakLoopExitAddress,
                [0x80088CF4] = WizardPeakResetAddress,
                [0x8008A204] = DeleteThunkAddress,
                [0x800880FC] = SoundThunkAddress
            });
        byte[] rewardPayload = Concat(Words(0x0260A021), relocatedRewardBody);
        if (rewardPayload.Length != RewardHandlerLength)
            throw new InvalidDataException($"Relocated reward payload is 0x{rewardPayload.Length:X}, expected 0x{RewardHandlerLength:X}.");

        byte[] rewardDispatch = Words(
            0x2402000D,
            EncodeBranch(0x04, 4, 2, RewardDispatchAddress + 4, RewardHandlerAddress),
            0,
            EncodeJump(WizardPeakLoopExitAddress),
            0);
        byte[] lockedDispatch = Words(
            0x240200AE,
            EncodeBranch(0x04, 4, 2, LockedDispatchAddress + 4, LockedHandlerAddress),
            0,
            EncodeJump(WizardPeakLoopExitAddress),
            0);
        byte[] deleteThunk = Words(0x0C01495A, 0x02602021, EncodeJump(WizardPeakLoopExitAddress), 0);
        byte[] soundThunk = Words(0x0C01569E, 0x24A70054, EncodeJump(WizardPeakLoopExitAddress), 0);

        byte[] payload = Concat(lockedPayload, rewardPayload, rewardDispatch, lockedDispatch, deleteThunk, soundThunk);
        if (payload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Wizard Peak handler payload is 0x{payload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        GuardSha256(payload, ExpectedHandlerPayloadSha256, "Wizard Peak handler bundle");
        return payload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, WizardPeakOverlayEntryOffset, OriginalOverlaySize);
        ...
        WriteWord(expanded, OverlayRelativeOffset(RewardDispatchHookAddress), RewardHandlerAddress);
        WriteWord(expanded, OverlayRelativeOffset(LockedDispatchHookAddress), LockedHandlerAddress);
        GuardWord(expanded, OverlayRelativeOffset(NativeKeyDispatchAddress), 0x80081AB0, "preserved Wizard Peak class 0x00AD dispatch pointer");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded WizardPeak overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<WizardPeakNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, WizardPeakDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, AppendedLockedChestPackageOffset + SourceCountRelativeOffset, SourceCountBefore, "retail Wizard Peak source count");
        GuardSha256(original.AsSpan(AppendedLockedChestPackageOffset, SceneSize).ToArray(), ExpectedRetailSceneSha256, "retail Wizard Peak scene preimage");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: WizardPeakDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: WizardPeakDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded WizardPeak data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0xF8, 4).ToArray(), "Wizard Peak actor root slot 0xF8");
        GuardBlank(expanded.AsSpan(0x1A4, 2).ToArray(), "Wizard Peak actor id slot 0x1A4");
        WriteUInt32(expanded, 0xF8, AppendedLockedChestPackageOffset);
        WriteUInt16(expanded, 0x1A4, 0x00AE);

        ShiftWizardPeakSceneFixupTail(expanded);
    ...
    private static void ShiftWizardPeakSceneFixupTail(byte[] expanded)
    {
        int scene = ExpandedSceneOffset;
        int countOffset = scene + ScenePointerFixupCountRelativeOffset;
        int listOffset = scene + ScenePointerFixupListRelativeOffset;
        GuardUInt32(expanded, countOffset, ScenePointerFixupCountBefore, "retail WizardPeak scene pointer-fixup count before tail insertion");

        int oldFixupEnd = ScenePointerFixupListRelativeOffset + (ScenePointerFixupCountBefore * 4);
        if (oldFixupEnd != SceneFixupTailInsertRelativeOffset)
            throw new InvalidDataException($"WizardPeak fixup tail starts at 0x{oldFixupEnd:X}, expected 0x{SceneFixupTailInsertRelativeOffset:X}.");

        byte[] shiftedSuffix = expanded.AsSpan(scene + SceneFixupTailInsertRelativeOffset, ShiftedSceneSuffixLength).ToArray();
        GuardSha256(shiftedSuffix, ExpectedShiftedSceneSuffixSha256, "WizardPeak live scene suffix before 0x1C fixup-tail insertion");
        GuardBlank(shiftedSuffix.AsSpan(0, 2).ToArray(), "WizardPeak live scene suffix leading alignment bytes");
        GuardBlank(expanded.AsSpan(scene + SceneFixupTailInsertRelativeOffset + ShiftedSceneSuffixLength, SceneZeroTailBefore).ToArray(),
            "WizardPeak scene zero tail before 0x1C insertion");

        uint maxFieldOffset = 0;
        for (int index = 0; index < ScenePointerFixupCountBefore; index++)
        {
            uint fieldOffset = ReadUInt32(expanded, listOffset + (index * 4));
            maxFieldOffset = Math.Max(maxFieldOffset, fieldOffset);
            if (fieldOffset >= SceneFixupTailInsertRelativeOffset || fieldOffset + 4 > SceneSize)
            {
                throw new InvalidDataException(
                    $"WizardPeak fixup #{index} field 0x{fieldOffset:X} is not wholly below the 0x{SceneFixupTailInsertRelativeOffset:X} insertion point.");
            }

            uint targetOffset = ReadUInt32(expanded, scene + checked((int)fieldOffset));
            if (targetOffset >= SceneFixupTailInsertRelativeOffset && targetOffset < SceneSize)
            {
                throw new InvalidDataException(
                    $"WizardPeak fixup #{index} points into the live suffix at 0x{targetOffset:X}; the guarded insertion profile is no longer valid.");
            }
        }
        if (maxFieldOffset != 0xDAA0)
            throw new InvalidDataException($"WizardPeak maximum existing fixup field is 0x{maxFieldOffset:X}, expected 0xDAA0.");

        Array.Copy(
            expanded,
            scene + SceneFixupTailInsertRelativeOffset,
            expanded,
            scene + SceneFixupTailInsertRelativeOffset + SceneFixupTailInsertionBytes,
            ShiftedSceneSuffixLength);
        Array.Clear(expanded, scene + SceneFixupTailInsertRelativeOffset, SceneFixupTailInsertionBytes);

        GuardEqual(
            expanded.AsSpan(scene + SceneFixupTailInsertRelativeOffset + SceneFixupTailInsertionBytes, ShiftedSceneSuffixLength).ToArray(),
            shiftedSuffix,
            "WizardPeak shifted live scene suffix");
        GuardBlank(
            expanded.AsSpan(
                scene + SceneFixupTailInsertRelativeOffset + SceneFixupTailInsertionBytes + ShiftedSceneSuffixLength,
                SceneZeroTailAfter).ToArray(),
            "WizardPeak scene zero tail after 0x1C insertion");
    }
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded WizardPeak source count");
        int rowPropertyPreimageOffset = sourceTableOffset + (KeyTrueIndex * RecordStride);
        int rowPropertyPreimageLength =
            (ExpandedSceneOffset + RewardMarkers[^1].PropsRelativeOffset + 0x28) - rowPropertyPreimageOffset;
        GuardSha256(expanded.AsSpan(rowPropertyPreimageOffset, rowPropertyPreimageLength).ToArray(),
            ExpectedRowPropertyPreimageSha256, "Wizard Peak T165-T171 source rows and property preimage");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "WizardPeak Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "WizardPeak Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"WizardPeak T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail WizardPeak scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "WizardPeak appended scene pointer-fixup slots");
        ...
        List<WizardPeakNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new WizardPeakNativeLockedChestTextureRegionPlan(
                region.Name,
                ...
                Sha256(targetTilePreimage), Sha256(targetPalettePreimage)));
        }
    }
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028009, "retail Wizard Peak copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x2442A8A0, "retail Wizard Peak copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(WizardPeakTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Wizard Peak treasure target is no longer 500.");
        WriteUInt16(bytes, WizardPeakTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                WizardPeakOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                WizardPeakDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[WizardPeakOverlayEntryIndex] != WizardPeakOverlayEntryOffset ||
            result[WizardPeakDataEntryIndex] != WizardPeakDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("WizardPeak entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                WizardPeakOverlayEntryIndex => finalOverlay,
                WizardPeakDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The WizardPeak candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                WizardPeakOverlayEntryIndex => ExpandedOverlaySize,
                WizardPeakDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[WizardPeakOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded WizardPeak overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[WizardPeakDataEntryIndex], finalData.Length), finalData, "expanded WizardPeak data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[WizardPeakOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[WizardPeakDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not WizardPeakOverlayEntryIndex and not WizardPeakDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[WizardPeakDataEntryIndex],
            ...
            actorLayout.NativeRootCount != 43 || actorLayout.LastNativeRoot != AppendedLockedChestPackageOffset)
        {
            throw new InvalidDataException($"Expanded WizardPeak actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028009, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x2442BA38, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(WizardPeakTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Wizard Peak treasure target did not read back as 510.");
        GuardEqual(
        ...
        int scene = ExpandedSceneOffset;
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final WizardPeak source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final WizardPeak pointer-fixup count");
        int appendedFixups = scene + ScenePointerFixupListRelativeOffset + (ScenePointerFixupCountBefore * 4);
        for (int index = KeyTrueIndex; index < SourceCountAfter; index++)
        {
            GuardUInt32(
                finalData,
                appendedFixups + ((index - KeyTrueIndex) * 4),
                checked((uint)(SourceTableRelativeOffset + (index * RecordStride))),
                $"final WizardPeak T{index} source-row fixup");
        }
        GuardSha256(
            finalData.AsSpan(
                scene + SceneFixupTailInsertRelativeOffset + SceneFixupTailInsertionBytes,
                ShiftedSceneSuffixLength).ToArray(),
            ExpectedShiftedSceneSuffixSha256,
            "final shifted WizardPeak live scene suffix");
        GuardBlank(
            finalData.AsSpan(
                scene + SceneFixupTailInsertRelativeOffset + SceneFixupTailInsertionBytes + ShiftedSceneSuffixLength,
                SceneZeroTailAfter).ToArray(),
            "final WizardPeak scene zero tail");
        GuardUInt32(finalData, 0xF8, AppendedLockedChestPackageOffset, "final Wizard Peak 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x1A4, 2)) != 0x00AE)
            throw new InvalidDataException("Final Wizard Peak actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; one 0x2800-byte WAD growth (0x1000 overlay + 0x1800 actor/model); SCUS relocation 53875->53880; exact native Peace Keepers 0x00AE Locked Chest plus missing 0x000D reward-handler closure and private delete/sound thunks; Wizard Peak-resident 0x00AD/debris dependencies; T165-T171 source rows/props and 191->198 pointer fixups; exact 0x1C live scene-suffix shift with byte-identical readback; four ownership-proven private texture/CLUT regions; treasure 500->510; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, WizardPeakOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(WizardPeakNativeLockedChestCandidateRequest request)
    {
