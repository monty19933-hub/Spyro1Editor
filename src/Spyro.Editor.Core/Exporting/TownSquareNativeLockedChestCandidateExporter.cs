...
public sealed record TownSquareNativeLockedChestCandidateRequest(
...
public sealed record TownSquareNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record TownSquareNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<TownSquareNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record TownSquareNativeLockedChestCandidateResult(
    string OutputImagePath,
    TownSquareNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Town Square transplant of Peace Keepers' native Key + Locked
/// Chest source bundle. Town Square owns the native Key and metal-debris
...
/// </summary>
public static class TownSquareNativeLockedChestCandidateExporter
{
    public const string RecipeId = "townsquare.peacekeepers.nativeLockedChest.rewardClosure.disposable.v1";
    ...
    private const int TownSquareOverlayEntryIndex = 15;
    private const long TownSquareOverlayEntryOffset = 0x118E800;
    private const int OriginalOverlaySize = 0xF800;
    private const int ExpandedOverlaySize = 0x10800;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x80089ECC;
    private const uint RelocatedCopyBufferAddress = 0x8008B238;

    private const int TownSquareDataEntryIndex = 16;
    private const long TownSquareDataEntryOffset = 0x119E000;
    private const int OriginalDataEntrySize = 0x2E2000;
    private const int ExpandedDataEntrySize = 0x2E3800;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x173000;
    private const int OriginalActorSubfileSize = 0x5D000;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1D0000;
    private const int ExpandedSceneOffset = 0x1D1800;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const uint RewardHandlerAddress = 0x8008A60C;
    private const int RewardHandlerLength = 0x438;
    private const uint RewardDispatchAddress = 0x8008AA44;
    private const uint LockedDispatchAddress = 0x8008AA58;
    private const uint DeleteThunkAddress = 0x8008AA6C;
    private const uint SoundThunkAddress = 0x8008AA7C;
    private const int OverlayPayloadOffset = 0xF494;
    private const int OverlayPayloadLength = 0xBC0;
    private const uint RewardDispatchHookAddress = 0x8007DB80;
    private const uint LockedDispatchHookAddress = 0x8007DC4C;
    private const uint TownSquareLoopExitAddress = 0x80087258;
    private const uint TownSquareResetAddress = 0x800858F8;
    ...
    private const int SourceCountRelativeOffset = 0x16C;
    private const int SourceTableRelativeOffset = 0x170;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 107;
    private const int SourceCountAfter = 114;
    private const int KeyTrueIndex = 107;
    private const int LockedChestTrueIndex = 108;
    private const int KeyPropsRelativeOffset = 0x28A0;
    private const int LockedChestPropsRelativeOffset = 0x28A4;
    private const int RewardPropsRelativeOffset = 0x28BC;
    private const int ScenePointerFixupCountRelativeOffset = 0x7F28;
    private const int ScenePointerFixupListRelativeOffset = 0x7F2C;
    private const int ScenePointerFixupCountBefore = 129;
    private const int ScenePointerFixupCountAfter = 136;

    private const int KeyRawX = 130150;
    private const int KeyRawY = 111032;
    private const int KeyRawZ = 11264;
    private const int LockedChestRawX = 132198;
    private const int LockedChestRawY = 111032;
    private const int LockedChestRawZ = 11264;

    private const int CopyBufferHiFileOffset = 0x4B098;
    private const int CopyBufferLoFileOffset = 0x4B09C;
    private const int TownSquareTreasureTableFileOffset = 0x5FC3E;
    ...
    private const string ExpectedRetailOverlaySha256 = "9bc923cc8d27703537b81b01f51fde351e58aabee87d38a8afc630f1031878a5";
    private const string ExpectedRetailDataEntrySha256 = "7ddbf6d9a6ee6c0f64c8564a89e374ca0ca234ef608ec812aae68e8176de1dc0";
    private const string ExpectedActorSubfileSha256 = "a37b7a8e5e5e660befed91e63c699a6c6d4b25ff8406f68b08e418c00d2a711d";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "2ab9f539e8903a18a59a254fa8dc21a186b4540e40376c1c1bd6d9055949b7c9";
    ...
    private const string ExpectedHandlerPayloadSha256 = "08f4372710b7162982397e247b75ed9b00cc5290ef9630d4e2fdd320611012b9";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "b312c20cbf425216c8b18f9a24fcee4398df45ef3be832846445db178427ae25";

    private static readonly int[] RewardMarkerTrueIndices = [109, 110, 111, 112, 113];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 109, 0x28BC, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 110, 0x28E4, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 111, 0x290C, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 112, 0x2934, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 113, 0x295C, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];

    private static readonly TextureRegion[] TextureRegions =
    [
        new("upper-lock", 0x000E, 192, 192, 0x4A2A, 0x0008, 0, 0, 0x1028,
        ...
        new("lower-lock", 0x000E, 192, 224, 0x4C2A, 0x0008, 0, 32, 0x1029,
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x001D, 96, 192, 0x102A,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x001F, 128, 128, 0x102B,
    ...
    public static TownSquareNativeLockedChestCandidateResult Export(TownSquareNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<TownSquareNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            TownSquareNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(13)}."
                    : "Disabled.",
                Verification: verification);

            return new TownSquareNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, TownSquareOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail TownSquare overlay");
        GuardSha256(ReadWad(source, layout, TownSquareDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail TownSquare data entry");
        GuardSha256(ReadWad(source, layout, TownSquareDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail TownSquare actor/model subfile");
        ...
        GuardSha256(ReadWad(source, layout, TownSquareOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail TownSquare overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, RewardDispatchHookAddress, 0x08021C96, "retail Town Square class 0x000D default jump");
        GuardWadWord(source, layout, RewardDispatchHookAddress + 4, 0, "retail Town Square class 0x000D delay");
        GuardWadWord(source, layout, LockedDispatchHookAddress, 0x08021C96, "retail Town Square class 0x00AE default jump");
        GuardWadWord(source, layout, LockedDispatchHookAddress + 4, 0, "retail Town Square class 0x00AE delay");
        GuardWadWord(source, layout, 0x8007DBD8, 0x240200AD, "retail Town Square Key compare");
        GuardWadWord(source, layout, 0x8007DBDC, 0x108217DB, "retail Town Square Key branch");
        GuardSha256(
            ReadWad(source, layout, TownSquareOverlayEntryOffset + (TownSquareResetAddress - OverlayLoadAddress) - 0x10, 0x40),
            "23aabbf138f06867c52f9a0c903236b5acf57bc39db7764c174b7d3d97c35dff",
            "retail Town Square reset continuation window");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail TownSquare copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028009, "retail Town Square copy-buffer HI16");
        GuardWord(copyPair, 4, 0x24429ECC, "retail Town Square copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, TownSquareTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Town Square treasure target is no longer the retail value 200.");
        }

        WadEntryLayout overlay = wad.ByIndex[TownSquareOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[TownSquareDataEntryIndex];
        if (overlay.Offset != TownSquareOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != TownSquareDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact TownSquare overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition townSquare = catalog.FindByKey("townsquare") ??
            throw new InvalidDataException("The level catalog does not contain TownSquare.");
        if (townSquare.SourceWadEntry != TownSquareDataEntryIndex || townSquare.LevelId != 13)
            throw new InvalidDataException("The TownSquare level catalog mapping changed.");

        NativeTexturePageOwnershipReport report = NativeTexturePageOwnershipScanner.Scan(sourcePath, townSquare);
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != TownSquareDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"TownSquare texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private TownSquare texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private TownSquare texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedLockedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = TownSquareLoopExitAddress });
        ...
        byte[] relocatedRewardBody = RelocateCode(
            ...
            {
                [0x8008A20C] = TownSquareLoopExitAddress,
                [0x80088CF4] = TownSquareResetAddress,
        ...
        byte[] rewardDispatch = Words(
            ...
            0,
            EncodeJump(TownSquareLoopExitAddress),
            0);
        byte[] lockedDispatch = Words(
            ...
            EncodeJump(TownSquareLoopExitAddress),
            0);
        byte[] deleteThunk = Words(0x0C01495A, 0x02602021, EncodeJump(TownSquareLoopExitAddress), 0);
        byte[] soundThunk = Words(0x0C01569E, 0x24A70054, EncodeJump(TownSquareLoopExitAddress), 0);
        ...
        if (payload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Town Square handler payload is 0x{payload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        GuardSha256(payload, ExpectedHandlerPayloadSha256, "Town Square handler bundle");
        return payload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, TownSquareOverlayEntryOffset, OriginalOverlaySize);
        ...
        WriteWord(expanded, OverlayRelativeOffset(RewardDispatchHookAddress), EncodeJump(RewardDispatchAddress));
        WriteWord(expanded, OverlayRelativeOffset(RewardDispatchHookAddress) + 4, 0);
        WriteWord(expanded, OverlayRelativeOffset(LockedDispatchHookAddress), EncodeJump(LockedDispatchAddress));
        WriteWord(expanded, OverlayRelativeOffset(LockedDispatchHookAddress) + 4, 0);
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded TownSquare overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<TownSquareNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, TownSquareDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, 0x1D0000 + SourceCountRelativeOffset, SourceCountBefore, "retail Town Square source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: TownSquareDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: TownSquareDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded TownSquare data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0xE4, 4).ToArray(), "Town Square actor root slot 0xE4");
        GuardBlank(expanded.AsSpan(0x19A, 2).ToArray(), "Town Square actor id slot 0x19A");
        WriteUInt32(expanded, 0xE4, AppendedLockedChestPackageOffset);
        WriteUInt16(expanded, 0x19A, 0x00AE);
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded TownSquare source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Town Square T107-T113 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "TownSquare Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "TownSquare Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"TownSquare T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail TownSquare scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "TownSquare appended scene pointer-fixup slots");
        ...
        List<TownSquareNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new TownSquareNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028009, "retail Town Square copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x24429ECC, "retail Town Square copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(TownSquareTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Town Square treasure target is no longer 200.");
        WriteUInt16(bytes, TownSquareTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                TownSquareOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                TownSquareDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[TownSquareOverlayEntryIndex] != TownSquareOverlayEntryOffset ||
            result[TownSquareDataEntryIndex] != TownSquareDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("TownSquare entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                TownSquareOverlayEntryIndex => finalOverlay,
                TownSquareDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The TownSquare candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                TownSquareOverlayEntryIndex => ExpandedOverlaySize,
                TownSquareDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[TownSquareOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded TownSquare overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[TownSquareDataEntryIndex], finalData.Length), finalData, "expanded TownSquare data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[TownSquareOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[TownSquareDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not TownSquareOverlayEntryIndex and not TownSquareDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[TownSquareDataEntryIndex],
            ...
            actorLayout.NativeRootCount != 38 || actorLayout.LastNativeRoot != AppendedLockedChestPackageOffset)
        {
            throw new InvalidDataException($"Expanded TownSquare actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028009, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x2442B238, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(TownSquareTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Town Square treasure target did not read back as 210.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final TownSquare source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final TownSquare pointer-fixup count");
        GuardUInt32(finalData, 0xE4, AppendedLockedChestPackageOffset, "final Town Square 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x19A, 2)) != 0x00AE)
            throw new InvalidDataException("Final Town Square actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; one 0x2800-byte WAD growth (0x1000 overlay + 0x1800 actor/model); SCUS relocation 53875->53880; exact native Peace Keepers 0x00AE Locked Chest plus missing 0x000D reward handler closure and private delete/sound thunks; Town Square-resident 0x00AD/debris dependencies; T107-T113 source rows/props/fixups; four ownership-proven private texture/CLUT regions; treasure 200->210; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, TownSquareOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(TownSquareNativeLockedChestCandidateRequest request)
    {
