...
public sealed record DreamWeaversNativeLockedChestCandidateRequest(
...
public sealed record DreamWeaversNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record DreamWeaversNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<DreamWeaversNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record DreamWeaversNativeLockedChestCandidateResult(
    string OutputImagePath,
    DreamWeaversNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Dream Weavers transplant of Peace Keepers' native Key + Locked Chest
/// source bundle. Dream Weavers already owns the retail Key dispatch, class 0x000D
...
/// </summary>
public static class DreamWeaversNativeLockedChestCandidateExporter
{
    public const string RecipeId = "dreamweavers.peacekeepers.nativeLockedChest.disposable.v1";
    ...
    private const int DreamWeaversOverlayEntryIndex = 57;
    private const long DreamWeaversOverlayEntryOffset = 0x43DB000;
    private const int OriginalOverlaySize = 0x11800;
    private const int ExpandedOverlaySize = 0x12000;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x8008BB38;
    private const uint RelocatedCopyBufferAddress = 0x8008C280;

    private const int DreamWeaversDataEntryIndex = 58;
    private const long DreamWeaversDataEntryOffset = 0x43EC800;
    private const int OriginalDataEntrySize = 0x28D800;
    private const int ExpandedDataEntrySize = 0x28F000;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x179000;
    private const int OriginalActorSubfileSize = 0x4C800;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1C5800;
    private const int ExpandedSceneOffset = 0x1C7000;
    private const int SceneSize = 0x14800;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const int OverlayPayloadOffset = 0x11100;
    private const int OverlayPayloadLength = LockedHandlerLength;
    private const uint NativeRewardDispatchAddress = 0x8007AB1C;
    private const uint NativeKeyDispatchAddress = 0x8007AD9C;
    private const uint DispatchHookAddress = 0x8007ADA0;
    private const uint DreamWeaversLoopExitAddress = 0x80088DD8;
    ...
    private const int SourceCountRelativeOffset = 0x6E60;
    private const int SourceTableRelativeOffset = 0x6E64;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 151;
    private const int SourceCountAfter = 158;
    private const int KeyTrueIndex = 151;
    private const int LockedChestTrueIndex = 152;
    private const int KeyPropsRelativeOffset = 0xA4C0;
    private const int LockedChestPropsRelativeOffset = 0xA4C4;
    private const int RewardPropsRelativeOffset = 0xA4DC;
    private const int ScenePointerFixupCountRelativeOffset = 0x14188;
    private const int ScenePointerFixupListRelativeOffset = 0x1418C;
    private const int ScenePointerFixupCountBefore = 0xB6;
    private const int ScenePointerFixupCountAfter = 0xBD;

    private const int KeyRawX = 97500;
    private const int KeyRawY = 78000;
    private const int KeyRawZ = 13824;
    private const int LockedChestRawX = 101000;
    private const int LockedChestRawY = 78000;
    private const int LockedChestRawZ = 13824;

    private const int CopyBufferHiFileOffset = 0x4BA38;
    private const int CopyBufferLoFileOffset = 0x4BA3C;
    private const int DreamWeaversTreasureTableFileOffset = 0x5FC68;
    private const int TreasureTargetBefore = 300;
    private const int TreasureTargetAfter = 310;
    ...
    private const string ExpectedRetailOverlaySha256 = "dcbe76c16c51393e784037af420c7fb7c00cecd526ed4d890ae5f1b257cdb352";
    private const string ExpectedRetailDataEntrySha256 = "cadd3a2366eae64995c0eb40427410c569b6cbc8faea1ba0872639f31230c559";
    private const string ExpectedActorSubfileSha256 = "4dd60f308a255479ce61f157e4f1d85a0f8b9e17c3978d8db2a7f3d834c22d87";
    private const string ExpectedRetailSceneSha256 = "07d17dc15d363b2f31f26840f877db10a0232e5fc43ad97138e9bec40565fe8e";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "028d0ad5a0b99e7fe07fc217074f03673b7df55307abf0e7d6b56c3bb28a5f58";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "26522848717bedc0fd0e6bc2c69b69a81ec4c5afcd91bf17fcf3ff309a468a33";
    ...
    private static readonly int[] RewardMarkerTrueIndices = [153, 154, 155, 156, 157];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 153, 0xA4DC, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 154, 0xA504, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 155, 0xA52C, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 156, 0xA554, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 157, 0xA57C, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];

    private static readonly TextureRegion[] TextureRegions =
    [
        new("upper-lock", 0x000E, 192, 192, 0x4A2A, 0x000C, 128, 224, 0x3430,
        ...
        new("lower-lock", 0x000E, 192, 224, 0x4C2A, 0x000C, 160, 224, 0x3431,
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x000C, 192, 224, 0x3432,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x000C, 224, 224, 0x3433,
    ...
    public static DreamWeaversNativeLockedChestCandidateResult Export(DreamWeaversNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<DreamWeaversNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            DreamWeaversNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(50)}."
                    : "Disabled.",
                Verification: verification);

            return new DreamWeaversNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, DreamWeaversOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail DreamWeavers overlay");
        GuardSha256(ReadWad(source, layout, DreamWeaversDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail DreamWeavers data entry");
        GuardSha256(ReadWad(source, layout, DreamWeaversDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail DreamWeavers actor/model subfile");
        GuardSha256(ReadWad(source, layout, DreamWeaversDataEntryOffset + AppendedLockedChestPackageOffset, SceneSize), ExpectedRetailSceneSha256, "retail Dream Weavers scene");
        ...
        GuardSha256(ReadWad(source, layout, DreamWeaversOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail DreamWeavers overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, DispatchHookAddress, DreamWeaversLoopExitAddress, "retail Dream Weavers class 0x00AE table slot");
        GuardWadWord(source, layout, DispatchHookAddress + 4, DreamWeaversLoopExitAddress, "retail Dream Weavers class 0x00AF neighbor table slot");
        GuardWadWord(source, layout, NativeKeyDispatchAddress, 0x800850EC, "retail Dream Weavers class 0x00AD Key table slot");
        GuardWadWord(source, layout, NativeRewardDispatchAddress, 0x8007E5BC, "retail Dream Weavers class 0x000D reward table slot");
        GuardWadWord(source, layout, DreamWeaversLoopExitAddress, 0x8FA90240, "retail Dream Weavers actor-loop exit");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail DreamWeavers copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028009, "retail DreamWeavers copy-buffer HI16");
        GuardWord(copyPair, 4, 0x2442BB38, "retail Dream Weavers copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, DreamWeaversTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Dream Weavers treasure target is no longer the retail value 300.");
        }

        WadEntryLayout overlay = wad.ByIndex[DreamWeaversOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[DreamWeaversDataEntryIndex];
        if (overlay.Offset != DreamWeaversOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != DreamWeaversDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact DreamWeavers overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition level = catalog.FindByKey("dreamweavers") ??
            throw new InvalidDataException("The level catalog does not contain Dream Weavers.");
        if (level.SourceWadEntry != DreamWeaversDataEntryIndex || level.LevelId != 50)
            throw new InvalidDataException("The Dream Weavers level catalog mapping changed.");
        ...
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != DreamWeaversDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"DreamWeavers texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private DreamWeavers texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private DreamWeavers texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
        if (selected.Count != 2176)
            throw new InvalidDataException($"Private Dream Weavers texture allocation covers {selected.Count:N0} bytes, expected exactly 2,176.");
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = DreamWeaversLoopExitAddress });
        ...
        if (lockedPayload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Dream Weavers handler payload is 0x{lockedPayload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        return lockedPayload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, DreamWeaversOverlayEntryOffset, OriginalOverlaySize);
        ...
        GuardWord(expanded, OverlayRelativeOffset(DispatchHookAddress) + 4, DreamWeaversLoopExitAddress, "preserved Dream Weavers class 0x00AF table slot");
        GuardWord(expanded, OverlayRelativeOffset(NativeKeyDispatchAddress), 0x800850EC, "preserved Dream Weavers Key table slot");
        GuardWord(expanded, OverlayRelativeOffset(NativeRewardDispatchAddress), 0x8007E5BC, "preserved Dream Weavers reward table slot");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded DreamWeavers overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<DreamWeaversNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, DreamWeaversDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, SourceCountRelativeOffset + AppendedLockedChestPackageOffset, SourceCountBefore, "retail Dream Weavers source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: DreamWeaversDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: DreamWeaversDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded DreamWeavers data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0xFC, 4).ToArray(), "Dream Weavers actor root slot 0xFC");
        GuardBlank(expanded.AsSpan(0x1A6, 2).ToArray(), "Dream Weavers actor id slot 0x1A6");
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded DreamWeavers source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Dream Weavers T151-T157 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "DreamWeavers Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "DreamWeavers Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"DreamWeavers T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail DreamWeavers scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "DreamWeavers appended scene pointer-fixup slots");
        ...
        List<DreamWeaversNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new DreamWeaversNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028009, "retail DreamWeavers copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x2442BB38, "retail Dream Weavers copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(DreamWeaversTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Dream Weavers treasure target is no longer 300.");
        WriteUInt16(bytes, DreamWeaversTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                DreamWeaversOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                DreamWeaversDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[DreamWeaversOverlayEntryIndex] != DreamWeaversOverlayEntryOffset ||
            result[DreamWeaversDataEntryIndex] != DreamWeaversDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("DreamWeavers entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                DreamWeaversOverlayEntryIndex => finalOverlay,
                DreamWeaversDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The DreamWeavers candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                DreamWeaversOverlayEntryIndex => ExpandedOverlaySize,
                DreamWeaversDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[DreamWeaversOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded DreamWeavers overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[DreamWeaversDataEntryIndex], finalData.Length), finalData, "expanded DreamWeavers data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[DreamWeaversOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[DreamWeaversDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not DreamWeaversOverlayEntryIndex and not DreamWeaversDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[DreamWeaversDataEntryIndex],
        ...
        {
            throw new InvalidDataException($"Expanded DreamWeavers actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028009, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x2442C280, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(DreamWeaversTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Dream Weavers treasure target did not read back as 310.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final DreamWeavers source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final DreamWeavers pointer-fixup count");
        GuardUInt32(finalData, 0xFC, AppendedLockedChestPackageOffset, "final Dream Weavers 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x1A6, 2)) != 0x00AE)
            throw new InvalidDataException("Final Dream Weavers actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; 0x800-byte overlay plus 0x1800-byte actor/model WAD growth; copy buffer moved from 0x8008BB38 to aligned 0x8008C280 after the transplanted handler; SCUS relocation 53875->53879; direct guarded class-0x00AE table entry with Dream Weavers-resident 0x00AD/0x000D/debris dependencies; T151-T157 source rows/props/fixups; four exact zero-preimage, ownership-proven private texture/CLUT regions; source-disc-verified Dream Weavers treasure target 300->310 at SCUS 0x5FC68; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final-image readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, DreamWeaversOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(DreamWeaversNativeLockedChestCandidateRequest request)
    {
