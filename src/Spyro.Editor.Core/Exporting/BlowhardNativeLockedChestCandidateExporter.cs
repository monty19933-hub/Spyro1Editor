...
public sealed record BlowhardNativeLockedChestCandidateRequest(
...
public sealed record BlowhardNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record BlowhardNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<BlowhardNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record BlowhardNativeLockedChestCandidateResult(
    string OutputImagePath,
    BlowhardNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Blowhard transplant of Peace Keepers' native Key + Locked Chest
/// source bundle. Blowhard already owns the retail Key dispatch, class 0x000D
...
/// </summary>
public static class BlowhardNativeLockedChestCandidateExporter
{
    public const string RecipeId = "blowhard.peacekeepers.nativeLockedChest.disposable.v1";
    ...
    private const int BlowhardOverlayEntryIndex = 41;
    private const long BlowhardOverlayEntryOffset = 0x31F5000;
    ...
    private const uint OriginalCopyBufferAddress = 0x8008749C;
    private const uint RelocatedCopyBufferAddress = 0x80087BF0;

    private const int BlowhardDataEntryIndex = 42;
    private const long BlowhardDataEntryOffset = 0x3202000;
    private const int OriginalDataEntrySize = 0x1DD800;
    private const int ExpandedDataEntrySize = 0x1DF000;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x121000;
    private const int OriginalActorSubfileSize = 0x4A000;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x16B000;
    private const int ExpandedSceneOffset = 0x16C800;
    ...
    private const int OverlayPayloadOffset = 0xCA64;
    private const int OverlayPayloadLength = LockedHandlerLength + DispatchShimLength;
    private const uint DispatchHookAddress = 0x8007B118;
    private const uint BlowhardLoopExitAddress = 0x80083A68;
    ...
    private const int SourceCountRelativeOffset = 0x1FDB8;
    private const int SourceTableRelativeOffset = 0x1FDBC;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 87;
    private const int SourceCountAfter = 94;
    private const int KeyTrueIndex = 87;
    private const int LockedChestTrueIndex = 88;
    private const int KeyPropsRelativeOffset = 0x21E10;
    private const int LockedChestPropsRelativeOffset = 0x21E14;
    private const int RewardPropsRelativeOffset = 0x21E2C;
    private const int ScenePointerFixupCountRelativeOffset = 0x26E94;
    private const int ScenePointerFixupListRelativeOffset = 0x26E98;
    private const int ScenePointerFixupCountBefore = 101;
    private const int ScenePointerFixupCountAfter = 108;

    private const int KeyRawX = 78000;
    private const int KeyRawY = 58000;
    private const int KeyRawZ = 12957;
    private const int LockedChestRawX = 80000;
    private const int LockedChestRawY = 58000;
    private const int LockedChestRawZ = 12957;

    private const int CopyBufferHiFileOffset = 0x4B6C0;
    private const int CopyBufferLoFileOffset = 0x4B6C4;
    private const int BlowhardTreasureTableFileOffset = 0x5FC58;
    ...
    private const string ExpectedRetailOverlaySha256 = "1ee0030607139491b6d076c389837d5b6c0a38e87d324fec2e14b58b47f95aef";
    private const string ExpectedRetailDataEntrySha256 = "3dfe32f742de381626afc133b7e750bb3b21a08ce51cae4ec4ef1eee296a8bf6";
    private const string ExpectedActorSubfileSha256 = "fbae55f2ddff005346cea50bd569714b2ae51ed8f22cfa63f267b949ff9325ce";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "028b760a921df157a7768c906f6a02751f7e4e070c30349fa61f8c740f8605a9";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "f2f5cd759e8d8e64c390f8f3805471e35248a4ea27cf913d2f9d067f6b788e5f";

    private static readonly int[] RewardMarkerTrueIndices = [89, 90, 91, 92, 93];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 89, 0x21E2C, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 90, 0x21E54, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 91, 0x21E7C, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 92, 0x21EA4, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 93, 0x21ECC, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];
    ...
    public static BlowhardNativeLockedChestCandidateResult Export(BlowhardNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<BlowhardNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            BlowhardNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(34)}."
                    : "Disabled.",
                Verification: verification);

            return new BlowhardNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, BlowhardOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail Blowhard overlay");
        GuardSha256(ReadWad(source, layout, BlowhardDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail Blowhard data entry");
        GuardSha256(ReadWad(source, layout, BlowhardDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail Blowhard actor/model subfile");
        ...
        GuardSha256(ReadWad(source, layout, BlowhardOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail Blowhard overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, 0x8007B02C, 0x2402000D, "retail Blowhard reward compare");
        GuardWadWord(source, layout, 0x8007B030, 0x10620146, "retail Blowhard reward branch");
        GuardWadWord(source, layout, 0x8007B104, 0x240200AD, "retail Blowhard Key compare");
        GuardWadWord(source, layout, 0x8007B108, 0x10620FF9, "retail Blowhard Key branch");
        GuardWadWord(source, layout, 0x8007B10C, 0x240200C2, "retail Blowhard class 0xC2 compare");
        GuardWadWord(source, layout, 0x8007B110, 0x10621094, "retail Blowhard class 0xC2 branch");
        GuardWadWord(source, layout, DispatchHookAddress, 0x08020E9A, "retail Blowhard class 0xAE default jump");
        GuardWadWord(source, layout, DispatchHookAddress + 4, 0, "retail Blowhard class 0xAE delay word");
        GuardWadWord(source, layout, BlowhardLoopExitAddress, 0x8FA90258, "retail Blowhard actor-loop exit");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail Blowhard copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028008, "retail Blowhard copy-buffer HI16");
        GuardWord(copyPair, 4, 0x2442749C, "retail Blowhard copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, BlowhardTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Blowhard treasure target is no longer the retail value 400.");
        }

        WadEntryLayout overlay = wad.ByIndex[BlowhardOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[BlowhardDataEntryIndex];
        if (overlay.Offset != BlowhardOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != BlowhardDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact Blowhard overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition blowhard = catalog.FindByKey("blowhard") ??
            throw new InvalidDataException("The level catalog does not contain Blowhard.");
        if (blowhard.SourceWadEntry != BlowhardDataEntryIndex || blowhard.LevelId != 34)
            throw new InvalidDataException("The Blowhard level catalog mapping changed.");

        NativeTexturePageOwnershipReport report = NativeTexturePageOwnershipScanner.Scan(sourcePath, blowhard);
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != BlowhardDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"Blowhard texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private Blowhard texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private Blowhard texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = BlowhardLoopExitAddress });
        ...
        byte[] dispatch = Words(
            ...
            0,
            EncodeJump(BlowhardLoopExitAddress),
            0);
        if (dispatch.Length != DispatchShimLength)
            throw new InvalidDataException("Blowhard Locked Chest dispatch shim length changed.");

        byte[] payload = Concat(lockedPayload, dispatch);
        if (payload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Blowhard handler payload is 0x{payload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        return payload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, BlowhardOverlayEntryOffset, OriginalOverlaySize);
        ...
        GuardWord(expanded, OverlayRelativeOffset(0x8007B108), 0x10620FF9, "preserved Blowhard Key branch");
        GuardWord(expanded, OverlayRelativeOffset(0x8007B110), 0x10621094, "preserved Blowhard class 0xC2 branch");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded Blowhard overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<BlowhardNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, BlowhardDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, SourceCountRelativeOffset + 0x16B000, SourceCountBefore, "retail Blowhard source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: BlowhardDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: BlowhardDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded Blowhard data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0xE4, 4).ToArray(), "Blowhard actor root slot 0xE4");
        GuardBlank(expanded.AsSpan(0x19A, 2).ToArray(), "Blowhard actor id slot 0x19A");
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded Blowhard source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Blowhard T87-T93 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "Blowhard Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "Blowhard Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"Blowhard T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail Blowhard scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "Blowhard appended scene pointer-fixup slots");
        ...
        List<BlowhardNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new BlowhardNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028008, "retail Blowhard copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x2442749C, "retail Blowhard copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(BlowhardTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Blowhard treasure target is no longer 400.");
        WriteUInt16(bytes, BlowhardTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                BlowhardOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                BlowhardDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[BlowhardOverlayEntryIndex] != BlowhardOverlayEntryOffset ||
            result[BlowhardDataEntryIndex] != BlowhardDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("Blowhard entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                BlowhardOverlayEntryIndex => finalOverlay,
                BlowhardDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The Blowhard candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                BlowhardOverlayEntryIndex => ExpandedOverlaySize,
                BlowhardDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[BlowhardOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded Blowhard overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[BlowhardDataEntryIndex], finalData.Length), finalData, "expanded Blowhard data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[BlowhardOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[BlowhardDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not BlowhardOverlayEntryIndex and not BlowhardDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[BlowhardDataEntryIndex],
        ...
        {
            throw new InvalidDataException($"Expanded Blowhard actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028008, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x24427BF0, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(BlowhardTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Blowhard treasure target did not read back as 410.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final Blowhard source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final Blowhard pointer-fixup count");
        GuardUInt32(finalData, 0xE4, AppendedLockedChestPackageOffset, "final Blowhard 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x19A, 2)) != 0x00AE)
            throw new InvalidDataException("Final Blowhard actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; one 0x2000-byte WAD growth (0x800 overlay + 0x1800 actor/model); copy buffer moved to 0x80087BF0 after the exact handler/shim payload; SCUS relocation 53875->53879; native Peace Keepers 0x00AE handler/model with Blowhard-resident 0x00AD/0x000D/debris dependencies; T87-T93 source rows/props/fixups; four ownership-proven private texture/CLUT regions; treasure 400->410; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, BlowhardOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(BlowhardNativeLockedChestCandidateRequest request)
    {
