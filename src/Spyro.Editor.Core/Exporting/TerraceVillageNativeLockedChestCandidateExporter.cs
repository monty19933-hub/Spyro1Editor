...
public sealed record TerraceVillageNativeLockedChestCandidateRequest(
...
public sealed record TerraceVillageNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record TerraceVillageNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<TerraceVillageNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record TerraceVillageNativeLockedChestCandidateResult(
    string OutputImagePath,
    TerraceVillageNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Terrace Village transplant of Peace Keepers' native Key + Locked Chest
/// source bundle. Terrace Village already owns the retail Key dispatch, class 0x000D
...
/// </summary>
public static class TerraceVillageNativeLockedChestCandidateExporter
{
    public const string RecipeId = "terracevillage.peacekeepers.nativeLockedChest.disposable.v1";
    ...
    private const int TerraceVillageOverlayEntryIndex = 47;
    private const long TerraceVillageOverlayEntryOffset = 0x37F2800;
    private const int OriginalOverlaySize = 0xD000;
    private const int ExpandedOverlaySize = 0xD800;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x80087944;
    private const uint RelocatedCopyBufferAddress = 0x80088090;

    private const int TerraceVillageDataEntryIndex = 48;
    private const long TerraceVillageDataEntryOffset = 0x37FF800;
    private const int OriginalDataEntrySize = 0x26E800;
    private const int ExpandedDataEntrySize = 0x270000;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x14F000;
    private const int OriginalActorSubfileSize = 0x74800;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1C3800;
    private const int ExpandedSceneOffset = 0x1C5000;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const int OverlayPayloadOffset = 0xCF0C;
    ...
    private const uint TerraceVillageLoopExitAddress = 0x80084610;
    ...
    private const int SourceCountRelativeOffset = 0x10988;
    private const int SourceTableRelativeOffset = 0x1098C;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 164;
    private const int SourceCountAfter = 171;
    private const int KeyTrueIndex = 164;
    private const int LockedChestTrueIndex = 165;
    private const int KeyPropsRelativeOffset = 0x14460;
    private const int LockedChestPropsRelativeOffset = 0x14464;
    private const int RewardPropsRelativeOffset = 0x1447C;
    private const int ScenePointerFixupCountRelativeOffset = 0x1AD94;
    private const int ScenePointerFixupListRelativeOffset = 0x1AD98;
    private const int ScenePointerFixupCountBefore = 207;
    private const int ScenePointerFixupCountAfter = 214;

    private const int KeyRawX = 58000;
    private const int KeyRawY = 96000;
    private const int KeyRawZ = 10240;
    private const int LockedChestRawX = 60048;
    private const int LockedChestRawY = 96000;
    private const int LockedChestRawZ = 10240;

    private const int CopyBufferHiFileOffset = 0x4B850;
    private const int CopyBufferLoFileOffset = 0x4B854;
    private const int TerraceVillageTreasureTableFileOffset = 0x5FC5E;
    ...
    private const string ExpectedRetailOverlaySha256 = "285fc63da6b536af2b41c19e41d1db03a980007226dc55e5d930a1998f39c50e";
    private const string ExpectedRetailDataEntrySha256 = "26bd02231f93d780ee75b528aac8e5797fbb0b578acfb2ba21e692872e6b5c5e";
    private const string ExpectedActorSubfileSha256 = "6ff1a192eb22173b9c88c95c1c58745e69e620d96ac5a30f98a9bd94643e7077";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "dc57843c6e57498ff073feca99efaa90baeb11df332adebd742b2b5218c531d9";
    ...
    private const string ExpectedHandlerPayloadSha256 = "c229cb1715c5b2f19129fa52e394fad4649d489dc25c63d5a3cd9d3c2c3bac6b";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "1c0a23e44e3f04ab3e52feef7c2ed35bed7bcfe7848bd49504ab302c6447f194";

    private static readonly int[] RewardMarkerTrueIndices = [166, 167, 168, 169, 170];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 166, 0x1447C, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 167, 0x144A4, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 168, 0x144CC, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 169, 0x144F4, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 170, 0x1451C, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];

    private static readonly TextureRegion[] TextureRegions =
    [
        new("upper-lock", 0x000E, 192, 192, 0x4A2A, 0x001E, 0, 64, 0x2E60,
        ...
        new("lower-lock", 0x000E, 192, 224, 0x4C2A, 0x001E, 32, 64, 0x2E61,
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x001E, 64, 64, 0x2E62,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x001E, 96, 64, 0x2E63,
    ...
    public static TerraceVillageNativeLockedChestCandidateResult Export(TerraceVillageNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<TerraceVillageNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            TerraceVillageNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(41)}."
                    : "Disabled.",
                Verification: verification);

            return new TerraceVillageNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, TerraceVillageOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail TerraceVillage overlay");
        GuardSha256(ReadWad(source, layout, TerraceVillageDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail TerraceVillage data entry");
        GuardSha256(ReadWad(source, layout, TerraceVillageDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail TerraceVillage actor/model subfile");
        ...
        GuardSha256(ReadWad(source, layout, TerraceVillageOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail TerraceVillage overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, DispatchHookAddress, TerraceVillageLoopExitAddress, "retail Terrace Village class 0xAE table default");
        GuardWadWord(source, layout, NativeKeyDispatchAddress, 0x8007F644, "retail Terrace Village Key table entry");
        GuardWadWord(source, layout, NativeRewardDispatchAddress, 0x8007B9E8, "retail Terrace Village reward table entry");
        GuardWadWord(source, layout, TerraceVillageLoopExitAddress, 0x8FAC0220, "retail Terrace Village actor-loop exit");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail TerraceVillage copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028008, "retail TerraceVillage copy-buffer HI16");
        GuardWord(copyPair, 4, 0x24427944, "retail Terrace Village copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, TerraceVillageTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Terrace Village treasure target is no longer the retail value 400.");
        }

        WadEntryLayout overlay = wad.ByIndex[TerraceVillageOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[TerraceVillageDataEntryIndex];
        if (overlay.Offset != TerraceVillageOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != TerraceVillageDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact TerraceVillage overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition level = catalog.FindByKey("terracevillage") ??
            throw new InvalidDataException("The level catalog does not contain Terrace Village.");
        if (level.SourceWadEntry != TerraceVillageDataEntryIndex || level.LevelId != 41)
            throw new InvalidDataException("The Terrace Village level catalog mapping changed.");
        ...
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != TerraceVillageDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"TerraceVillage texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private TerraceVillage texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private TerraceVillage texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = TerraceVillageLoopExitAddress });
        ...
        if (lockedPayload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Terrace Village handler payload is 0x{lockedPayload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        GuardSha256(lockedPayload, ExpectedHandlerPayloadSha256, "relocated Terrace Village Locked Chest handler payload");
        return lockedPayload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, TerraceVillageOverlayEntryOffset, OriginalOverlaySize);
        ...
        GuardWord(expanded, OverlayRelativeOffset(NativeKeyDispatchAddress), 0x8007F644, "preserved Terrace Village Key table entry");
        GuardWord(expanded, OverlayRelativeOffset(NativeRewardDispatchAddress), 0x8007B9E8, "preserved Terrace Village reward table entry");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded TerraceVillage overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<TerraceVillageNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, TerraceVillageDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, SourceCountRelativeOffset + 0x1C3800, SourceCountBefore, "retail Terrace Village source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: TerraceVillageDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: TerraceVillageDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded TerraceVillage data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0x100, 4).ToArray(), "Terrace Village actor root slot 0x100");
        GuardBlank(expanded.AsSpan(0x1A8, 2).ToArray(), "Terrace Village actor id slot 0x1A8");
        WriteUInt32(expanded, 0x100, AppendedLockedChestPackageOffset);
        WriteUInt16(expanded, 0x1A8, 0x00AE);
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded TerraceVillage source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Terrace Village T164-T170 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "TerraceVillage Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "TerraceVillage Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"TerraceVillage T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail TerraceVillage scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "TerraceVillage appended scene pointer-fixup slots");
        ...
        List<TerraceVillageNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new TerraceVillageNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028008, "retail TerraceVillage copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x24427944, "retail Terrace Village copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(TerraceVillageTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Terrace Village treasure target is no longer 400.");
        WriteUInt16(bytes, TerraceVillageTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                TerraceVillageOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                TerraceVillageDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[TerraceVillageOverlayEntryIndex] != TerraceVillageOverlayEntryOffset ||
            result[TerraceVillageDataEntryIndex] != TerraceVillageDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("TerraceVillage entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                TerraceVillageOverlayEntryIndex => finalOverlay,
                TerraceVillageDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The TerraceVillage candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                TerraceVillageOverlayEntryIndex => ExpandedOverlaySize,
                TerraceVillageDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[TerraceVillageOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded TerraceVillage overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[TerraceVillageDataEntryIndex], finalData.Length), finalData, "expanded TerraceVillage data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[TerraceVillageOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[TerraceVillageDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not TerraceVillageOverlayEntryIndex and not TerraceVillageDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[TerraceVillageDataEntryIndex],
            ...
            actorLayout.NativeRootCount != 45 || actorLayout.LastNativeRoot != AppendedLockedChestPackageOffset)
        {
            throw new InvalidDataException($"Expanded TerraceVillage actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028009, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x24428090, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(TerraceVillageTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Terrace Village treasure target did not read back as 410.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final TerraceVillage source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final TerraceVillage pointer-fixup count");
        GuardUInt32(finalData, 0x100, AppendedLockedChestPackageOffset, "final Terrace Village 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x1A8, 2)) != 0x00AE)
            throw new InvalidDataException("Final Terrace Village actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; one 0x2000-byte WAD growth (0x800 overlay + 0x1800 actor/model); copy buffer moved to 0x80088090; SCUS relocation 53875->53879; native Peace Keepers 0x00AE handler/model with Terrace Village-resident 0x00AD/0x000D/debris dependencies; T164-T170 source rows/props/fixups; four ownership-proven private texture/CLUT regions; treasure 400->410; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, TerraceVillageOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(TerraceVillageNativeLockedChestCandidateRequest request)
    {
