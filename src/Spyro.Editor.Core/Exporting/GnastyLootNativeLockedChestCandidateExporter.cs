...
public sealed record GnastyLootNativeLockedChestCandidateRequest(
...
public sealed record GnastyLootNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record GnastyLootNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<GnastyLootNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record GnastyLootNativeLockedChestCandidateResult(
    string OutputImagePath,
    GnastyLootNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Gnasty's Loot transplant of Peace Keepers' native Key + Locked Chest
/// source bundle. Gnasty's Loot already owns the retail Key dispatch, class 0x000D
...
/// </summary>
public static class GnastyLootNativeLockedChestCandidateExporter
{
    public const string RecipeId = "gnastysloot.peacekeepers.nativeLockedChest.disposable.v1";
    ...
    private const int GnastyLootOverlayEntryIndex = 77;
    private const long GnastyLootOverlayEntryOffset = 0x5AEA800;
    private const int OriginalOverlaySize = 0xC000;
    private const int ExpandedOverlaySize = 0xC000;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x80086264;
    private const uint RelocatedCopyBufferAddress = OverlayLoadAddress + ExpandedOverlaySize;

    private const int GnastyLootDataEntryIndex = 78;
    private const long GnastyLootDataEntryOffset = 0x5AF6800;
    private const int OriginalDataEntrySize = 0x1A1000;
    private const int ExpandedDataEntrySize = 0x1A2800;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x165800;
    private const int OriginalActorSubfileSize = 0x29000;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x18E800;
    private const int ExpandedSceneOffset = 0x190000;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const int OverlayPayloadOffset = 0xB82C;
    private const int OverlayPayloadLength = LockedHandlerLength;
    private const uint DispatchHookAddress = 0x8007AF28;
    private const uint NativeKeyDispatchAddress = 0x8007AEC0;
    private const uint NativeRewardDispatchAddress = 0x8007AE6C;
    private const uint GnastyLootLoopExitAddress = 0x80083644;
    ...
    private const int SourceCountRelativeOffset = 0x5B68;
    private const int SourceTableRelativeOffset = 0x5B6C;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 129;
    private const int SourceCountAfter = 136;
    private const int KeyTrueIndex = 129;
    private const int LockedChestTrueIndex = 130;
    private const int KeyPropsRelativeOffset = 0x8A30;
    private const int LockedChestPropsRelativeOffset = 0x8A34;
    private const int RewardPropsRelativeOffset = 0x8A4C;
    private const int ScenePointerFixupCountRelativeOffset = 0x1250C;
    private const int ScenePointerFixupListRelativeOffset = 0x12510;
    private const int ScenePointerFixupCountBefore = 0x92;
    private const int ScenePointerFixupCountAfter = 0x99;

    private const int KeyRawX = 92500;
    private const int KeyRawY = 43000;
    private const int KeyRawZ = 14848;
    private const int LockedChestRawX = 94000;
    private const int LockedChestRawY = 43000;
    private const int LockedChestRawZ = 14848;

    private const int CopyBufferHiFileOffset = 0x4BE40;
    private const int CopyBufferLoFileOffset = 0x4BE44;
    private const int GnastyLootTreasureTableFileOffset = 0x5FC7C;
    private const int TreasureTargetBefore = 2000;
    private const int TreasureTargetAfter = 2010;
    ...
    private const string ExpectedRetailOverlaySha256 = "2117b3b14116b98035d9b003df3f299ef127ee34558f700af2be9fcb7e165a6e";
    private const string ExpectedRetailDataEntrySha256 = "259295b2d2feff2b8a9bfe14431be52df43fbc2b59218ae2af7510f42c4f4ad9";
    private const string ExpectedActorSubfileSha256 = "325edee4ef46bdb04dd6c67eb44ad72d5ecf5da2d87e5d50f5093073b5f55f78";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "539e453f6567332a91eacea2c4fb214f1613d9165e4ff7b6ec1026da73d5898e";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "d9323a9702d4dfe2434eec0025657bccd54780727e62fe27e83b396f69e6d833";

    private static readonly int[] RewardMarkerTrueIndices = [131, 132, 133, 134, 135];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 131, 0x8A4C, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 132, 0x8A74, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 133, 0x8A9C, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 134, 0x8AC4, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 135, 0x8AEC, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];
    ...
    [
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x0009, 0, 0, 0x1024,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x0009, 0, 32, 0x1027,
    ...
    public static GnastyLootNativeLockedChestCandidateResult Export(GnastyLootNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<GnastyLootNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            GnastyLootNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(64)}."
                    : "Disabled.",
                Verification: verification);

            return new GnastyLootNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, GnastyLootOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail GnastyLoot overlay");
        GuardSha256(ReadWad(source, layout, GnastyLootDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail GnastyLoot data entry");
        GuardSha256(ReadWad(source, layout, GnastyLootDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail GnastyLoot actor/model subfile");
        ...
        GuardSha256(ReadWad(source, layout, GnastyLootOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail GnastyLoot overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, DispatchHookAddress, 0x08020D91, "retail Gnasty's Loot class 0xAE default jump");
        GuardWadWord(source, layout, DispatchHookAddress + 4, 0, "retail Gnasty's Loot class 0xAE delay word");
        GuardWadWord(source, layout, NativeKeyDispatchAddress - 4, 0x240200AD, "retail Gnasty's Loot Key compare");
        GuardWadWord(source, layout, NativeKeyDispatchAddress, 0x10820C31, "retail Gnasty's Loot Key branch");
        GuardWadWord(source, layout, NativeRewardDispatchAddress - 4, 0x2402000D, "retail Gnasty's Loot reward compare");
        GuardWadWord(source, layout, NativeRewardDispatchAddress, 0x10820143, "retail Gnasty's Loot reward branch");
        GuardWadWord(source, layout, GnastyLootLoopExitAddress, 0x8FA90240, "retail Gnasty's Loot actor-loop exit");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail GnastyLoot copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028008, "retail GnastyLoot copy-buffer HI16");
        GuardWord(copyPair, 4, 0x24426264, "retail Gnasty's Loot copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, GnastyLootTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Gnasty's Loot treasure target is no longer the retail value 2000.");
        }

        WadEntryLayout overlay = wad.ByIndex[GnastyLootOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[GnastyLootDataEntryIndex];
        if (overlay.Offset != GnastyLootOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != GnastyLootDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact GnastyLoot overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition level = catalog.FindByKey("gnastysloot") ??
            throw new InvalidDataException("The level catalog does not contain Gnasty's Loot.");
        if (level.SourceWadEntry != GnastyLootDataEntryIndex || level.LevelId != 64)
            throw new InvalidDataException("The Gnasty's Loot level catalog mapping changed.");
        ...
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != GnastyLootDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"GnastyLoot texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private GnastyLoot texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private GnastyLoot texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = GnastyLootLoopExitAddress });
        ...
        if (lockedPayload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Gnasty's Loot handler payload is 0x{lockedPayload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        return lockedPayload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, GnastyLootOverlayEntryOffset, OriginalOverlaySize);
        ...
        WriteWord(expanded, OverlayRelativeOffset(DispatchHookAddress), EncodeJump(LockedHandlerAddress));
        GuardWord(expanded, OverlayRelativeOffset(DispatchHookAddress) + 4, 0, "preserved Gnasty's Loot class 0xAE delay word");
        GuardWord(expanded, OverlayRelativeOffset(NativeKeyDispatchAddress), 0x10820C31, "preserved Gnasty's Loot Key branch");
        GuardWord(expanded, OverlayRelativeOffset(NativeRewardDispatchAddress), 0x10820143, "preserved Gnasty's Loot reward branch");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded GnastyLoot overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<GnastyLootNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, GnastyLootDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, SourceCountRelativeOffset + 0x18E800, SourceCountBefore, "retail Gnasty's Loot source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: GnastyLootDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: GnastyLootDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded GnastyLoot data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0xE8, 4).ToArray(), "Gnasty's Loot actor root slot 0xE8");
        GuardBlank(expanded.AsSpan(0x19C, 2).ToArray(), "Gnasty's Loot actor id slot 0x19C");
        WriteUInt32(expanded, 0xE8, AppendedLockedChestPackageOffset);
        WriteUInt16(expanded, 0x19C, 0x00AE);
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded GnastyLoot source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Gnasty's Loot T129-T135 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "GnastyLoot Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "GnastyLoot Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"GnastyLoot T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail GnastyLoot scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "GnastyLoot appended scene pointer-fixup slots");
        ...
        List<GnastyLootNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new GnastyLootNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028008, "retail GnastyLoot copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x24426264, "retail Gnasty's Loot copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(GnastyLootTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Gnasty's Loot treasure target is no longer 2000.");
        WriteUInt16(bytes, GnastyLootTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                GnastyLootOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                GnastyLootDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[GnastyLootOverlayEntryIndex] != GnastyLootOverlayEntryOffset ||
            result[GnastyLootDataEntryIndex] != GnastyLootDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("GnastyLoot entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                GnastyLootOverlayEntryIndex => finalOverlay,
                GnastyLootDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The GnastyLoot candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                GnastyLootOverlayEntryIndex => ExpandedOverlaySize,
                GnastyLootDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[GnastyLootOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded GnastyLoot overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[GnastyLootDataEntryIndex], finalData.Length), finalData, "expanded GnastyLoot data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[GnastyLootOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[GnastyLootDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not GnastyLootOverlayEntryIndex and not GnastyLootDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[GnastyLootDataEntryIndex],
            ...
            actorLayout.NativeRootCount != 39 || actorLayout.LastNativeRoot != AppendedLockedChestPackageOffset)
        {
            throw new InvalidDataException($"Expanded GnastyLoot actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028008, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x24426A38, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(GnastyLootTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Gnasty's Loot treasure target did not read back as 2010.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final GnastyLoot source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final GnastyLoot pointer-fixup count");
        GuardUInt32(finalData, 0xE8, AppendedLockedChestPackageOffset, "final Gnasty's Loot 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x19C, 2)) != 0x00AE)
            throw new InvalidDataException("Final Gnasty's Loot actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; one 0x1800-byte actor/model WAD growth with no overlay growth; copy buffer moved to overlay end 0x80086A38; SCUS relocation 53875->53878; native Peace Keepers 0x00AE handler/model with Gnasty's Loot-resident 0x00AD/0x000D/debris dependencies; T129-T135 source rows/props/fixups; four ownership-proven private texture/CLUT regions; treasure 2000->2010; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, GnastyLootOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(GnastyLootNativeLockedChestCandidateRequest request)
    {
