...
internal static class UnusedLevel65StandaloneSpawnRuntimeCandidateExporter
{
    ...
    public const string ExpectedRawSectorDiffSha256 =
        "fba0516661b96af2668991e30fb619b492a774de2d20536f734bcf7a126536d4";
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int lba = 0; lba < sectorCount; lba++)
        {
            ...
            int header = CountDiff(left, right, 0, 12);
            int subheader = CountDiff(left, right, 12, 8);
            int payload = CountDiff(left, right, 24, 2048);
            int edc = CountDiff(left, right, 2072, 4);
            int reserved = CountDiff(left, right, 2076, 8);
            int eccP = CountDiff(left, right, 2084, 172);
            int eccQ = CountDiff(left, right, 2256, 96);
