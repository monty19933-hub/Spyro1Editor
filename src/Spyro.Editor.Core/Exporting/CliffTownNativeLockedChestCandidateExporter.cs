...
public sealed record CliffTownNativeLockedChestCandidateRequest(
...
public sealed record CliffTownNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record CliffTownNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<CliffTownNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record CliffTownNativeLockedChestCandidateResult(
    string OutputImagePath,
    CliffTownNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Cliff Town transplant of Peace Keepers' native Key + Locked Chest
/// source bundle. Cliff Town already owns the retail Key dispatch, class 0x000D
...
/// </summary>
public static class CliffTownNativeLockedChestCandidateExporter
{
    public const string RecipeId = "clifftown.peacekeepers.nativeLockedChest.disposable.v1";
    ...
    private const int CliffTownOverlayEntryIndex = 25;
    private const long CliffTownOverlayEntryOffset = 0x1E02000;
    private const int OriginalOverlaySize = 0x11800;
    private const int ExpandedOverlaySize = 0x11800;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x8008BAF8;
    private const uint RelocatedCopyBufferAddress = 0x8008C240;

    private const int CliffTownDataEntryIndex = 26;
    private const long CliffTownDataEntryOffset = 0x1E13800;
    private const int OriginalDataEntrySize = 0x286800;
    private const int ExpandedDataEntrySize = 0x288000;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x15B800;
    private const int OriginalActorSubfileSize = 0x70800;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1CC000;
    private const int ExpandedSceneOffset = 0x1CD800;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const int OverlayPayloadOffset = 0x110C0;
    private const int OverlayPayloadLength = LockedHandlerLength;
    private const uint DispatchHookAddress = 0x8007AD80;
    private const uint NativeKeyDispatchAddress = 0x8007AD7C;
    private const uint NativeRewardDispatchAddress = 0x8007AAFC;
    private const uint CliffTownLoopExitAddress = 0x80088B3C;
    ...
    private const int SourceCountRelativeOffset = 0x204;
    private const int SourceTableRelativeOffset = 0x208;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 167;
    private const int SourceCountAfter = 174;
    private const int KeyTrueIndex = 167;
    private const int LockedChestTrueIndex = 168;
    private const int KeyPropsRelativeOffset = 0x3DE0;
    private const int LockedChestPropsRelativeOffset = 0x3DE4;
    private const int RewardPropsRelativeOffset = 0x3DFC;
    private const int ScenePointerFixupCountRelativeOffset = 0x9868;
    private const int ScenePointerFixupListRelativeOffset = 0x986C;
    private const int ScenePointerFixupCountBefore = 210;
    private const int ScenePointerFixupCountAfter = 217;

    private const int KeyRawX = 145000;
    private const int KeyRawY = 48000;
    private const int KeyRawZ = 15360;
    private const int LockedChestRawX = 148000;
    private const int LockedChestRawY = 48000;
    private const int LockedChestRawZ = 15360;

    private const int CopyBufferHiFileOffset = 0x4B358;
    private const int CopyBufferLoFileOffset = 0x4B35C;
    private const int CliffTownTreasureTableFileOffset = 0x5FC48;
    ...
    private const string ExpectedRetailOverlaySha256 = "25f0fc287cc8b6b5b70d960630025e009e0e425f7af723dadf008605af5b0286";
    private const string ExpectedRetailDataEntrySha256 = "8246261aa9d5fe9f83bc138051bbfc9734af4673928808b08d1b414674babdc0";
    private const string ExpectedActorSubfileSha256 = "3bf1014aaa9247cfb3bb3692941b5126ee1555f2b29cda4668a7e6408b50b5e9";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "c4b1e4de5d1ba837ba3ffa47f5b23fcfc9352096948734b0b3d5f6d1f75f440a";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "f22fadad2d75a4b72a7903c1ec767ff5e39cf67cb50bc05f0e69139a410948d9";

    private static readonly int[] RewardMarkerTrueIndices = [169, 170, 171, 172, 173];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 169, 0x3DFC, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 170, 0x3E24, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 171, 0x3E4C, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 172, 0x3E74, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 173, 0x3E9C, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];
    ...
    [
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x0008, 0, 192, 0x1024,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x0008, 32, 192, 0x1025,
    ...
    public static CliffTownNativeLockedChestCandidateResult Export(CliffTownNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<CliffTownNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            CliffTownNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(22)}."
                    : "Disabled.",
                Verification: verification);

            return new CliffTownNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, CliffTownOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail CliffTown overlay");
        GuardSha256(ReadWad(source, layout, CliffTownDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail CliffTown data entry");
        GuardSha256(ReadWad(source, layout, CliffTownDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail CliffTown actor/model subfile");
        ...
        GuardSha256(ReadWad(source, layout, CliffTownOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail CliffTown overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, DispatchHookAddress, 0x80088B3C, "retail Cliff Town class 0xAE jump-table target");
        GuardWadWord(source, layout, NativeKeyDispatchAddress, 0x8008389C, "retail Cliff Town Key jump-table target");
        GuardWadWord(source, layout, NativeRewardDispatchAddress, 0x8007E64C, "retail Cliff Town reward jump-table target");
        GuardWadWord(source, layout, CliffTownLoopExitAddress, 0x8FA901E0, "retail Cliff Town actor-loop exit");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail CliffTown copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028009, "retail CliffTown copy-buffer HI16");
        GuardWord(copyPair, 4, 0x2442BAF8, "retail Cliff Town copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, CliffTownTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Cliff Town treasure target is no longer the retail value 400.");
        }

        WadEntryLayout overlay = wad.ByIndex[CliffTownOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[CliffTownDataEntryIndex];
        if (overlay.Offset != CliffTownOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != CliffTownDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact CliffTown overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition level = catalog.FindByKey("clifftown") ??
            throw new InvalidDataException("The level catalog does not contain Cliff Town.");
        if (level.SourceWadEntry != CliffTownDataEntryIndex || level.LevelId != 22)
            throw new InvalidDataException("The Cliff Town level catalog mapping changed.");
        ...
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != CliffTownDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"CliffTown texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private CliffTown texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private CliffTown texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = CliffTownLoopExitAddress });
        ...
        if (lockedPayload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Cliff Town handler payload is 0x{lockedPayload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        return lockedPayload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, CliffTownOverlayEntryOffset, OriginalOverlaySize);
        ...
        GuardWord(expanded, OverlayRelativeOffset(NativeKeyDispatchAddress), 0x8008389C, "preserved Cliff Town Key jump-table target");
        GuardWord(expanded, OverlayRelativeOffset(NativeRewardDispatchAddress), 0x8007E64C, "preserved Cliff Town reward jump-table target");
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded CliffTown overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<CliffTownNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, CliffTownDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, 0x1CC000 + SourceCountRelativeOffset, SourceCountBefore, "retail Cliff Town source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: CliffTownDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: CliffTownDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded CliffTown data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0xFC, 4).ToArray(), "Cliff Town actor root slot 0xFC");
        GuardBlank(expanded.AsSpan(0x1A6, 2).ToArray(), "Cliff Town actor id slot 0x1A6");
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded CliffTown source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Cliff Town T167-T173 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "CliffTown Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "CliffTown Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"CliffTown T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail CliffTown scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "CliffTown appended scene pointer-fixup slots");
        ...
        List<CliffTownNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new CliffTownNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028009, "retail CliffTown copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x2442BAF8, "retail Cliff Town copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(CliffTownTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Cliff Town treasure target is no longer 400.");
        WriteUInt16(bytes, CliffTownTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                CliffTownOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                CliffTownDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[CliffTownOverlayEntryIndex] != CliffTownOverlayEntryOffset ||
            result[CliffTownDataEntryIndex] != CliffTownDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("CliffTown entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                CliffTownOverlayEntryIndex => finalOverlay,
                CliffTownDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The CliffTown candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                CliffTownOverlayEntryIndex => ExpandedOverlaySize,
                CliffTownDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[CliffTownOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded CliffTown overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[CliffTownDataEntryIndex], finalData.Length), finalData, "expanded CliffTown data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[CliffTownOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[CliffTownDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not CliffTownOverlayEntryIndex and not CliffTownDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[CliffTownDataEntryIndex],
        ...
        {
            throw new InvalidDataException($"Expanded CliffTown actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028009, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x2442C240, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(CliffTownTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Cliff Town treasure target did not read back as 410.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final CliffTown source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final CliffTown pointer-fixup count");
        GuardUInt32(finalData, 0xFC, AppendedLockedChestPackageOffset, "final Cliff Town 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x1A6, 2)) != 0x00AE)
            throw new InvalidDataException("Final Cliff Town actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; one 0x1800-byte actor/model WAD growth with no overlay growth; copy buffer moved just past the overlay to 0x8008C240; SCUS relocation 53875->53878; native Peace Keepers 0x00AE handler/model with Cliff Town-resident 0x00AD/0x000D/debris dependencies; T167-T173 source rows/props/fixups; four ownership-proven private texture/CLUT regions; treasure 400->410; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, CliffTownOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(CliffTownNativeLockedChestCandidateRequest request)
    {
