...
public sealed record MistyBogNativeLockedChestCandidateRequest(
...
public sealed record MistyBogNativeLockedChestTextureRegionPlan(
    string Name,
    string SourceTpage,
    int SourceU,
    int SourceV,
...
public sealed record MistyBogNativeLockedChestCandidatePlan(
    ...
    IReadOnlyList<MistyBogNativeLockedChestTextureRegionPlan> TextureRegions,
...
public sealed record MistyBogNativeLockedChestCandidateResult(
    string OutputImagePath,
    MistyBogNativeLockedChestCandidatePlan Plan,
    bool Verified,
    long OutputLength,
    string Verification);

/// <summary>
/// Research-only Misty Bog transplant of Peace Keepers' native Key + Locked
/// Chest source bundle. Misty Bog owns the native Key and metal-debris
...
/// </summary>
public static class MistyBogNativeLockedChestCandidateExporter
{
    public const string RecipeId = "mistybog.peacekeepers.nativeLockedChest.rewardClosure.disposable.v1";
    ...
    private const int MistyBogOverlayEntryIndex = 49;
    private const long MistyBogOverlayEntryOffset = 0x3A6E000;
    private const int OriginalOverlaySize = 0xC800;
    private const int ExpandedOverlaySize = 0xD800;
    private const uint OverlayLoadAddress = 0x8007AA38;
    private const uint OriginalCopyBufferAddress = 0x80087130;
    private const uint RelocatedCopyBufferAddress = 0x80088238;

    private const int MistyBogDataEntryIndex = 50;
    private const long MistyBogDataEntryOffset = 0x3A7A800;
    private const int OriginalDataEntrySize = 0x2FE800;
    private const int ExpandedDataEntrySize = 0x300000;
    private const int ActorSubfileIndex = 2;
    private const int ActorSubfileOffset = 0x18E000;
    private const int OriginalActorSubfileSize = 0x4A800;
    private const int ActorSubfileGrowth = 0x1800;
    private const int AppendedLockedChestPackageOffset = 0x1D8800;
    private const int ExpandedSceneOffset = 0x1DA000;

    private const uint LockedHandlerAddress = OriginalCopyBufferAddress;
    private const int LockedHandlerLength = 0x740;
    private const uint RewardHandlerAddress = 0x80087870;
    private const int RewardHandlerLength = 0x438;
    private const uint RewardDispatchAddress = 0x80087CA8;
    private const uint LockedDispatchAddress = 0x80087CBC;
    private const uint DeleteThunkAddress = 0x80087CD0;
    private const uint SoundThunkAddress = 0x80087CE0;
    private const int OverlayPayloadOffset = 0xC6F8;
    private const int OverlayPayloadLength = 0xBC0;
    private const uint RewardDispatchHookAddress = 0x8007B0E8;
    private const uint LockedDispatchHookAddress = 0x8007B188;
    private const uint MistyBogLoopExitAddress = 0x800846CC;
    private const uint MistyBogResetAddress = 0x800814A8;
    ...
    private const int SourceCountRelativeOffset = 0x2724;
    private const int SourceTableRelativeOffset = 0x2728;
    private const int RecordStride = 0x58;
    private const int SourceCountBefore = 212;
    private const int SourceCountAfter = 219;
    private const int KeyTrueIndex = 212;
    private const int LockedChestTrueIndex = 213;
    private const int KeyPropsRelativeOffset = 0x7270;
    private const int LockedChestPropsRelativeOffset = 0x7274;
    private const int RewardPropsRelativeOffset = 0x728C;
    private const int ScenePointerFixupCountRelativeOffset = 0xE8C4;
    private const int ScenePointerFixupListRelativeOffset = 0xE8C8;
    private const int ScenePointerFixupCountBefore = 289;
    private const int ScenePointerFixupCountAfter = 296;

    private const int KeyRawX = 82000;
    private const int KeyRawY = 68000;
    private const int KeyRawZ = 4037;
    private const int LockedChestRawX = 84048;
    private const int LockedChestRawY = 68000;
    private const int LockedChestRawZ = 4037;

    private const int CopyBufferHiFileOffset = 0x4B8A0;
    private const int CopyBufferLoFileOffset = 0x4B8A4;
    private const int MistyBogTreasureTableFileOffset = 0x5FC60;
    ...
    private const string ExpectedRetailOverlaySha256 = "3b37eb8b4b0f5fb74e7d263670705e20c807f2d8f1bbf03f34a461eb1c3e2ff6";
    private const string ExpectedRetailDataEntrySha256 = "b8019233e082408247c0a5a7b5c60f95d36147d3029041aec60fa5decce44541";
    private const string ExpectedActorSubfileSha256 = "47d894c04754977e5e7330eef3072fd66a2e4911e20e43539f8caec8fda61ff1";
    private const string ExpectedExecutableSha256 = "a533d75cab8afaae6107ec35a02a9a5fe979a92c7c955f9cf1ee50f693a1b998";
    private const string ExpectedOverlayTailSha256 = "c6335809a3e1d53eaf59b98fee6ea092326c2543e886bb40616b71c6efcb998c";
    ...
    private const string ExpectedHandlerPayloadSha256 = "4e416ab0dd16e405f5902866cf2677c3f04c5f7cf9c8f76e3e1479d8cac172be";
    ...
    private const string ExpectedExecutableCopyPairSha256 = "f3cf29c701c5ef007aca1d5de46317d4ec2b73c79fb0197957643d455883bdbb";

    private static readonly int[] RewardMarkerTrueIndices = [214, 215, 216, 217, 218];

    private static readonly RewardMarkerPlacement[] RewardMarkers =
    [
        new(118, 214, 0x728C, -410, 1229, 512, "adeea114e991c3f418677f74ad63db11c026c68a393944b1855fc91bbd518e92"),
        new(119, 215, 0x72B4, 0, 1229, 512, "1ba45fde32011f52fbdf5e8b790142ff675f10703bbd92c0570de30da9500d00"),
        new(120, 216, 0x72DC, -615, 205, 512, "f631f41d5115bf6c37b345afd651fbb2a83d7cbb881ed063f63c1d687f424b9d"),
        new(121, 217, 0x7304, -410, 615, 512, "fb1e813085977d7506314bf576c969da306292545747534c72cb73d3427755f3"),
        new(122, 218, 0x732C, 204, 819, 512, "2ba846af549e6b158c6a0f0a98fe506b184c88b8c6fbc3ea24140be82e1dc1d8")
    ];

    private static readonly TextureRegion[] TextureRegions =
    [
        new("upper-lock", 0x000E, 192, 192, 0x4A2A, 0x001E, 96, 32, 0x1560,
        ...
        new("lower-lock", 0x000E, 192, 224, 0x4C2A, 0x001E, 128, 32, 0x1561,
        ...
        new("chest-body-a", 0x000F, 0, 160, 0x5C2A, 0x001E, 160, 32, 0x1562,
        ...
        new("chest-body-b", 0x000F, 0, 192, 0x5E2A, 0x001E, 192, 32, 0x1563,
    ...
    public static MistyBogNativeLockedChestCandidateResult Export(MistyBogNativeLockedChestCandidateRequest request)
    {
        ...
        try
        {
            ...
            byte[] rebasedPackage;
            int rebasedFaces;
            List<MistyBogNativeLockedChestTextureRegionPlan> regionPlans = [];
            ...
            MistyBogNativeLockedChestCandidatePlan plan = new(
                ...
                FastEntryInstructions: request.EnableFastEntry
                    ? $"Inventory: {TestLevelWarpPatch.ActivationSequence}; then {TestLevelWarpPatch.TargetSelectionText(42)}."
                    : "Disabled.",
                Verification: verification);

            return new MistyBogNativeLockedChestCandidateResult(
                outputPath,
                plan,
                Verified: true,
    ...
    private static void GuardRetailSource(string sourcePath, DiscLayout layout, WadArchiveLayout wad)
    {
        ...
        GuardSha256(ReadWad(source, layout, MistyBogOverlayEntryOffset, OriginalOverlaySize), ExpectedRetailOverlaySha256, "retail MistyBog overlay");
        GuardSha256(ReadWad(source, layout, MistyBogDataEntryOffset, OriginalDataEntrySize), ExpectedRetailDataEntrySha256, "retail MistyBog data entry");
        GuardSha256(ReadWad(source, layout, MistyBogDataEntryOffset + ActorSubfileOffset, OriginalActorSubfileSize), ExpectedActorSubfileSha256, "retail MistyBog actor/model subfile");
        ...
        GuardSha256(ReadWad(source, layout, MistyBogOverlayEntryOffset + OverlayPayloadOffset, OriginalOverlaySize - OverlayPayloadOffset), ExpectedOverlayTailSha256, "retail MistyBog overlay tail/copy-buffer marker");

        GuardWadWord(source, layout, RewardDispatchHookAddress, 0x080211B3, "retail Misty Bog class 0x000D default jump");
        GuardWadWord(source, layout, RewardDispatchHookAddress + 4, 0, "retail Misty Bog class 0x000D delay");
        GuardWadWord(source, layout, LockedDispatchHookAddress, 0x080211B3, "retail Misty Bog class 0x00AE default jump");
        GuardWadWord(source, layout, LockedDispatchHookAddress + 4, 0, "retail Misty Bog class 0x00AE delay");
        GuardWadWord(source, layout, 0x8007B17C, 0x240200AD, "retail Misty Bog Key compare");
        GuardWadWord(source, layout, 0x8007B180, 0x1082129C, "retail Misty Bog Key branch");
        GuardSha256(
            ReadWad(source, layout, MistyBogOverlayEntryOffset + (MistyBogResetAddress - OverlayLoadAddress) - 0x10, 0x40),
            "245ab8105eff3658bdff0fcac4b3900fc4cb58a5549e5743ded29dd7f10b501b",
            "retail Misty Bog reset continuation window");

        byte[] copyPair = DiscImage.ReadFileBytes(source, layout, executable.Lba, CopyBufferHiFileOffset, 8);
        GuardSha256(copyPair, ExpectedExecutableCopyPairSha256, "retail MistyBog copy-buffer pair");
        GuardWord(copyPair, 0, 0x3C028008, "retail Misty Bog copy-buffer HI16");
        GuardWord(copyPair, 4, 0x24427130, "retail Misty Bog copy-buffer LO16");
        if (BinaryPrimitives.ReadUInt16LittleEndian(
                DiscImage.ReadFileBytes(source, layout, executable.Lba, MistyBogTreasureTableFileOffset, 2)) != TreasureTargetBefore)
        {
            throw new InvalidDataException("The Misty Bog treasure target is no longer the retail value 500.");
        }

        WadEntryLayout overlay = wad.ByIndex[MistyBogOverlayEntryIndex];
        WadEntryLayout data = wad.ByIndex[MistyBogDataEntryIndex];
        if (overlay.Offset != MistyBogOverlayEntryOffset || overlay.Size != OriginalOverlaySize ||
            data.Offset != MistyBogDataEntryOffset || data.Size != OriginalDataEntrySize)
        {
            throw new InvalidDataException("The WAD analysis no longer identifies the exact MistyBog overlay/data entries.");
        }
    }
    ...
    private static string GuardTextureAllocation(string sourcePath, string catalogRoot)
    {
        ...
        LevelDefinition mistyBog = catalog.FindByKey("mistybog") ??
            throw new InvalidDataException("The level catalog does not contain MistyBog.");
        if (mistyBog.SourceWadEntry != MistyBogDataEntryIndex || mistyBog.LevelId != 42)
            throw new InvalidDataException("The MistyBog level catalog mapping changed.");

        NativeTexturePageOwnershipReport report = NativeTexturePageOwnershipScanner.Scan(sourcePath, mistyBog);
        if (!report.AllConsumerClosureComplete || !report.ProvablyPrivatePixelAndClutSpace ||
            ...
            report.TexturePagesWadOffset != MistyBogDataEntryOffset + TexturePagesSubfileOffset)
        {
            throw new InvalidDataException(
                $"MistyBog texture ownership is not closed for private allocation: {string.Join("; ", report.SafetyBlockers)}");
        }
        ...
        foreach (TextureRegion region in TextureRegions)
        {
                ...
                .Concat(BuildPaletteOffsets(region.TargetClut)))
            {
                if (!selected.Add(offset))
                    throw new InvalidDataException($"Private MistyBog texture allocation overlaps itself at 0x{offset:X}.");
                ...
                {
                    throw new InvalidDataException(
                        $"Private MistyBog texture allocation touches protected {protectedRange.Owner} at 0x{offset:X}.");
                }
            }
        }
    ...
    private static byte[] BuildHandlerPayload(FileStream retail, DiscLayout layout)
    {
        ...
        byte[] relocatedLockedBody = RelocateCode(
            lockedSegments,
            new Dictionary<uint, uint> { [0x8008A20C] = MistyBogLoopExitAddress });
        ...
        byte[] relocatedRewardBody = RelocateCode(
            ...
            {
                [0x8008A20C] = MistyBogLoopExitAddress,
                [0x80088CF4] = MistyBogResetAddress,
        ...
        byte[] rewardDispatch = Words(
            ...
            0,
            EncodeJump(MistyBogLoopExitAddress),
            0);
        byte[] lockedDispatch = Words(
            ...
            EncodeJump(MistyBogLoopExitAddress),
            0);
        byte[] deleteThunk = Words(0x0C01495A, 0x02602021, EncodeJump(MistyBogLoopExitAddress), 0);
        byte[] soundThunk = Words(0x0C01569E, 0x24A70054, EncodeJump(MistyBogLoopExitAddress), 0);
        ...
        if (payload.Length != OverlayPayloadLength)
            throw new InvalidDataException($"Misty Bog handler payload is 0x{payload.Length:X}, expected 0x{OverlayPayloadLength:X}.");
        GuardSha256(payload, ExpectedHandlerPayloadSha256, "Misty Bog handler bundle");
        return payload;
    }

    private static byte[] BuildFinalOverlay(FileStream retail, DiscLayout layout, byte[] handlerPayload)
    {
        byte[] original = ReadWad(retail, layout, MistyBogOverlayEntryOffset, OriginalOverlaySize);
        ...
        if (expanded.AsSpan(OverlayPayloadOffset + handlerPayload.Length).ToArray().Any(value => value != 0))
            throw new InvalidDataException("Expanded MistyBog overlay padding is not zero.");
        return expanded;
    }
    ...
    private static (byte[] Entry, byte[] RebasedPackage, int RebasedFaces) BuildFinalDataEntry(
        ...
        List<MistyBogNativeLockedChestTextureRegionPlan> regionPlans)
    {
        byte[] original = ReadWad(retail, layout, MistyBogDataEntryOffset, OriginalDataEntrySize);
        GuardUInt32(original, 0x1D8800 + SourceCountRelativeOffset, SourceCountBefore, "retail Misty Bog source count");
        ...
        NativeSkyRelocationPayload payload = new(
            PatchIndex: 0,
            WadLba: WadLba,
            OriginalWadOffset: MistyBogDataEntryOffset + ActorSubfileOffset + OriginalActorSubfileSize,
            StorageWadEntry: MistyBogDataEntryIndex,
        ...
        if (expanded.Length != ExpandedDataEntrySize)
            throw new InvalidDataException($"Expanded MistyBog data entry is 0x{expanded.Length:X}, expected 0x{ExpandedDataEntrySize:X}.");
        ...
        GuardBlank(expanded.AsSpan(0xFC, 4).ToArray(), "Misty Bog actor root slot 0xFC");
        GuardBlank(expanded.AsSpan(0x1A6, 2).ToArray(), "Misty Bog actor id slot 0x1A6");
        WriteUInt32(expanded, 0xFC, AppendedLockedChestPackageOffset);
        WriteUInt16(expanded, 0x1A6, 0x00AE);
    ...
    private static void ImportSourceRowsAndProps(FileStream retail, DiscLayout layout, byte[] expanded)
    {
        ...
        GuardUInt32(expanded, sourceCountOffset, SourceCountBefore, "expanded MistyBog source count");
        GuardBlank(expanded.AsSpan(sourceTableOffset + (KeyTrueIndex * RecordStride),
            (SourceCountAfter - SourceCountBefore) * RecordStride).ToArray(), "Misty Bog T212-T218 source rows");
        ...
        CopyGuardedBlank(expanded, ExpandedSceneOffset + KeyPropsRelativeOffset, keyProps, "MistyBog Key props");
        CopyGuardedBlank(expanded, ExpandedSceneOffset + LockedChestPropsRelativeOffset, chestProps, "MistyBog Locked Chest props");
        ...
        foreach (RewardMarkerPlacement marker in RewardMarkers)
        {
            ...
            CopyGuardedBlank(expanded, ExpandedSceneOffset + marker.PropsRelativeOffset, props,
                $"MistyBog T{marker.OutputTrueIndex} reward props");
        }

        WriteUInt32(expanded, sourceCountOffset, SourceCountAfter);
        GuardUInt32(expanded, pointerFixupCountOffset, ScenePointerFixupCountBefore, "retail MistyBog scene pointer-fixup count");
        ...
        GuardBlank(expanded.AsSpan(appendFixupOffset, (ScenePointerFixupCountAfter - ScenePointerFixupCountBefore) * 4).ToArray(),
            "MistyBog appended scene pointer-fixup slots");
        ...
        List<MistyBogNativeLockedChestTextureRegionPlan> regionPlans)
    {
        ...
        foreach (TextureRegion region in TextureRegions)
        {
            ...
            regionPlans.Add(new MistyBogNativeLockedChestTextureRegionPlan(
                region.Name,
    ...
    private static byte[] BuildFinalExecutable(FileStream retail, DiscLayout layout, bool enableFastEntry)
    {
        ...
        GuardWord(bytes, CopyBufferHiFileOffset, 0x3C028008, "retail Misty Bog copy-buffer HI16");
        GuardWord(bytes, CopyBufferLoFileOffset, 0x24427130, "retail Misty Bog copy-buffer LO16");
        ...
        if (BinaryPrimitives.ReadUInt16LittleEndian(bytes.AsSpan(MistyBogTreasureTableFileOffset, 2)) != TreasureTargetBefore)
            throw new InvalidDataException("The Misty Bog treasure target is no longer 500.");
        WriteUInt16(bytes, MistyBogTreasureTableFileOffset, TreasureTargetAfter);
    ...
    private static IReadOnlyDictionary<int, long> BuildRelocatedEntryOffsets(WadArchiveLayout wad)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                MistyBogOverlayEntryIndex => ExpandedOverlaySize - OriginalOverlaySize,
                MistyBogDataEntryIndex => ExpandedDataEntrySize - OriginalDataEntrySize,
                _ => 0
            };
        ...
        if (result[MistyBogOverlayEntryIndex] != MistyBogOverlayEntryOffset ||
            result[MistyBogDataEntryIndex] != MistyBogDataEntryOffset + (ExpandedOverlaySize - OriginalOverlaySize))
        {
            throw new InvalidDataException("MistyBog entry relocation offsets changed unexpectedly.");
        }
        return result;
    }
        ...
        byte[] finalExecutable)
    {
        ...
        foreach (WadEntryLayout entry in wad.Entries.OrderBy(entry => entry.Offset))
        {
            ...
            {
                MistyBogOverlayEntryIndex => finalOverlay,
                MistyBogDataEntryIndex => finalData,
        ...
        bool fastEntryEnabled)
    {
        ...
        if (new FileInfo(sourcePath).Length != new FileInfo(outputPath).Length)
            throw new InvalidDataException("The MistyBog candidate changed the raw disc-image length.");
        ...
        foreach (WadEntryLayout entry in wad.Entries)
        {
            ...
            {
                MistyBogOverlayEntryIndex => ExpandedOverlaySize,
                MistyBogDataEntryIndex => ExpandedDataEntrySize,
                _ => entry.Size
            };
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[MistyBogOverlayEntryIndex], finalOverlay.Length), finalOverlay, "expanded MistyBog overlay readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[MistyBogDataEntryIndex], finalData.Length), finalData, "expanded MistyBog data readback");
        ...
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[MistyBogOverlayEntryIndex] + OverlayPayloadOffset, handlerPayload.Length), handlerPayload, "Locked Chest handler payload readback");
        GuardEqual(ReadWad(output, layout, relocatedEntryOffsets[MistyBogDataEntryIndex] + AppendedLockedChestPackageOffset, rebasedPackage.Length), rebasedPackage, "Locked Chest package readback");

        foreach (WadEntryLayout entry in wad.Entries.Where(entry => entry.Index is not MistyBogOverlayEntryIndex and not MistyBogDataEntryIndex))
        {
        ...
        if (!CrossLevelActorPackageLayoutSafety.TryReadLayout(
                ...
                layout,
                WadLba,
                relocatedEntryOffsets[MistyBogDataEntryIndex],
            ...
            actorLayout.NativeRootCount != 44 || actorLayout.LastNativeRoot != AppendedLockedChestPackageOffset)
        {
            throw new InvalidDataException($"Expanded MistyBog actor/model layout failed validation: {actorReason}");
        }

        GuardWord(finalExecutable, CopyBufferHiFileOffset, 0x3C028009, "relocated copy-buffer HI16 readback");
        GuardWord(finalExecutable, CopyBufferLoFileOffset, 0x24428238, "relocated copy-buffer LO16 readback");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalExecutable.AsSpan(MistyBogTreasureTableFileOffset, 2)) != TreasureTargetAfter)
            throw new InvalidDataException("Misty Bog treasure target did not read back as 510.");
        GuardEqual(
        ...
        GuardUInt32(finalData, scene + SourceCountRelativeOffset, SourceCountAfter, "final MistyBog source count");
        GuardUInt32(finalData, scene + ScenePointerFixupCountRelativeOffset, ScenePointerFixupCountAfter, "final MistyBog pointer-fixup count");
        GuardUInt32(finalData, 0xFC, AppendedLockedChestPackageOffset, "final Misty Bog 0x00AE root");
        if (BinaryPrimitives.ReadUInt16LittleEndian(finalData.AsSpan(0x1A6, 2)) != 0x00AE)
            throw new InvalidDataException("Final Misty Bog actor ID slot does not contain 0x00AE.");

        return "Verified exact-USA retail preimages; one 0x2800-byte WAD growth (0x1000 overlay + 0x1800 actor/model); SCUS relocation 53875->53880; exact native Peace Keepers 0x00AE Locked Chest plus missing 0x000D reward handler closure and private delete/sound thunks; Misty Bog-resident 0x00AD/debris dependencies; T212-T218 source rows/props/fixups; four ownership-proven private texture/CLUT regions; treasure 500->510; optional guarded fast entry; untouched unrelated WAD entries; and byte-identical final readback. Runtime evidence is still required before editor promotion.";
    }
    ...
    private static void GuardWadWord(FileStream source, DiscLayout layout, uint runtimeAddress, uint expected, string label) =>
        GuardWord(ReadWad(source, layout, MistyBogOverlayEntryOffset + OverlayRelativeOffset(runtimeAddress), 4), 0, expected, label);
    ...
    private static void ValidateRequest(MistyBogNativeLockedChestCandidateRequest request)
    {
