...
Require(
    baseHashBefore == UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.BaseImageSha256 &&
    cueHashBefore == UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.BaseCueSha256,
...
if (publishFinal)
{
    string finalOutput = Path.Combine(
        ...
        "_local",
        "v5-stone-hill-level-replacement",
        UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.OutputDirectoryName);
    UnusedLevel65DragonSaveTriggerRuntimeCandidateResult published =
        await UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreateAsync(new(
            root,
            baseImage,
            baseCue,
            finalOutput,
    ...
    Require(
        UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ExpectedFinalReceiptSha256 == "PENDING" ||
        published.ArtifactHashes.ReceiptSha256 ==
            UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ExpectedFinalReceiptSha256,
        "The final-path canonical receipt hash changed.");
    Console.WriteLine("PUBLISHED ID65 dragon/save-trigger no-card runtime candidate");
...
string temporaryParent = Path.Combine(
    ...
    "spyro-id65-dragon-save-trigger-smoke-" + Guid.NewGuid().ToString("N"));
...
try
{
    ...
    try
    {
        _ = UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreatePaths(invalidOutput);
    }
    ...
    if (!OperatingSystem.IsWindows())
    {
        ...
        try
        {
            _ = await UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreateAsync(new(
                root,
                baseImage,
                baseCue,
                aliasedOutput));
        ...
        string leaseSymlinkOutput = Path.Combine(
            leaseSymlinkParent,
            UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.OutputDirectoryName);
        UnusedLevel65DragonSaveTriggerRuntimeCandidatePaths leaseSymlinkPaths =
            UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreatePaths(leaseSymlinkOutput);
    ...
    Require(wrongHashRejected && !Directory.Exists(wrongOutput),
        "A stale/wrong base produced a dragon save-trigger candidate.");
    ...
    string output = Path.Combine(
        publicationParent,
        UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.OutputDirectoryName);
    UnusedLevel65DragonSaveTriggerRuntimeCandidateRequest firstRequest = new(
        root,
        baseImage,
        baseCue,
        output,
        ReplaceExistingCandidate: false);
    UnusedLevel65DragonSaveTriggerRuntimeCandidateResult first =
        await UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreateAsync(firstRequest);
    ...
    UnusedLevel65DragonSaveTriggerRuntimeCandidateRequest deterministicRequest = new(
        root,
        baseImage,
        baseCue,
        output,
        ...
        TestStageHook: stage =>
        {
            ...
            if (!releaseWriter.Wait(TimeSpan.FromSeconds(30)))
                throw new TimeoutException("The smoke did not release the held dragon save-trigger writer.");
        });
    Task<UnusedLevel65DragonSaveTriggerRuntimeCandidateResult> replacementTask =
        Task.Run(() => UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreateAsync(deterministicRequest));
    ...
    try
    {
        _ = await UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreateAsync(
            firstRequest with { ReplaceExistingCandidate = true });
    }
    catch (IOException ex) when (
        ex.Message == UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ActiveWriterLeaseMessage)
    {
    ...
    Require(contentionRejected, "A concurrent dragon save-trigger writer was not rejected fail-closed.");
    UnusedLevel65DragonSaveTriggerRuntimeCandidateResult second = await replacementTask;
    ...
    Require(firstSnapshot == secondSnapshot, "A deterministic dragon save-trigger replacement changed any of seven files.");
    ...
    UnusedLevel65DragonSaveTriggerRuntimeCandidateResult handoff =
        await UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreateAsync(
            firstRequest with { ReplaceExistingCandidate = true });
    ...
    Require(
        UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter
    ...
    string recoveryOutput = Path.Combine(
        recoveryParent,
        UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.OutputDirectoryName);
    UnusedLevel65DragonSaveTriggerRuntimeCandidatePaths recoveryPaths =
        UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.CreatePaths(recoveryOutput);
    ...
    await ExpectVerificationFailureAsync(
        second.Paths.ConstructionPlanPath,
        bytes => [.. bytes, (byte)' '],
        second.Paths,
        "construction plan tamper");
    await ExpectVerificationFailureAsync(
    ...
    await ExpectCoordinatedArtifactAndReceiptHashMutationFailureAsync(
        second.Paths.OutputCuePath,
        bytes => [.. bytes, (byte)'\n'],
        second.Paths,
        "outputCueSha256",
        "coordinated CUE plus receipt-hash tamper");
    await ExpectCoordinatedArtifactAndReceiptHashMutationFailureAsync(
        second.Paths.RuntimeChecklistPath,
        bytes => [.. bytes, (byte)' '],
        second.Paths,
        "runtimeChecklistSha256",
        "coordinated checklist plus receipt-hash tamper");
    await ExpectCoordinatedArtifactAndReceiptHashMutationFailureAsync(
        second.Paths.FinderHelperPath,
        bytes => [.. bytes, .. Encoding.UTF8.GetBytes("# coordinated-tamper\n")],
        second.Paths,
        "finderHelperSha256",
        "coordinated Finder helper plus receipt-hash tamper");
    ...
    try
    {
        _ = await UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.VerifyPublishedAsync(second.Paths);
    }
    ...
    UnusedLevel65DragonSaveTriggerRuntimeCandidateResult finalReadback =
        await UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.VerifyPublishedAsync(second.Paths);
    ...
    Console.WriteLine(
        "PASS UnusedLevel65DragonSaveTriggerRuntimeCandidateSmoke: " +
        "direct locked-base T100/T101/T105 + camera translation; slot-35 dragon target 0->4; " +
        "350-byte/five-sector MODE2 readback; deterministic seven-file publication; persistent OS-lease contention/handoff, " +
    ...
    Console.WriteLine($"TEMP pathBoundReceiptSha256={finalReadback.ArtifactHashes.ReceiptSha256}");
    Console.WriteLine($"PIN constructionPlanSha256={finalReadback.ArtifactHashes.ConstructionPlanSha256}");
...
static void VerifyResult(
    UnusedLevel65DragonSaveTriggerRuntimeCandidateResult result,
    bool expectRecovery)
{
    UnusedLevel65DragonSaveTriggerPlan plan = result.Plan;
    UnusedLevel65DragonSaveTriggerReceipt receipt = result.Receipt;
    Require(plan.ProfileId == UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ProfileId &&
            plan.BaseImageSha256 == UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.BaseImageSha256 &&
            plan.BaseCueSha256 == UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.BaseCueSha256 &&
            plan.SourceDataSha256 == UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.SourceDataSha256 &&
            plan.SourceExecutableSha256 ==
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.SourceExecutableSha256 &&
            IsPinnedOrPending(
                plan.OutputDataSha256,
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.OutputDataSha256) &&
            IsPinnedOrPending(
                plan.OutputExecutableSha256,
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.OutputExecutableSha256),
        "The direct locked-base candidate identity changed.");
    Require(plan.RowPatches.Count == 3 &&
            plan.RowPatches.Select(row => row.TrueIndex).SequenceEqual(new[] { 100, 101, 105 }) &&
            plan.RowPatches.All(row => row.OnlyXyzChanged) &&
            plan.RowPatches[0].Before == new UnusedLevel65DragonSaveTriggerPoint(129_638, 123_085, 13_712) &&
            plan.RowPatches[0].After == new UnusedLevel65DragonSaveTriggerPoint(129_894, 123_341, 13_672) &&
            plan.RowPatches[1].Before == new UnusedLevel65DragonSaveTriggerPoint(129_638, 123_085, 13_835) &&
            plan.RowPatches[1].After == new UnusedLevel65DragonSaveTriggerPoint(129_894, 123_341, 13_795) &&
            plan.RowPatches[2].Before == new UnusedLevel65DragonSaveTriggerPoint(129_638, 123_085, 14_736) &&
            plan.RowPatches[2].After == new UnusedLevel65DragonSaveTriggerPoint(129_894, 123_341, 14_696),
        "The exact atomic T100/T101/T105 row witness changed.");
    Require(plan.CameraLeadPatch.WadOffset == 0x6B0C600 &&
            plan.CameraLeadPatch.FrameCount == 1 &&
            plan.CameraLeadPatch.FrameStride == 0x44 &&
            plan.CameraLeadPatch.XyzByteLengthPerFrame == 12 &&
            plan.CameraLeadPatch.ChangedLogicalBytes == 3 &&
            plan.CameraLeadPatch.BeforeSha256 ==
                "443ab95f3cbcbbfcc2f0918c4556e3c306d20ceafa7f882b76b150e30828348e" &&
            plan.CameraLeadPatch.AfterSha256 ==
            ...
            plan.CameraFrameTrackPatch.WadOffset == 0x6C17D68 &&
            plan.CameraFrameTrackPatch.FrameCount == 112 &&
            plan.CameraFrameTrackPatch.FrameStride == 24 &&
            plan.CameraFrameTrackPatch.XyzByteLengthPerFrame == 12 &&
            ...
            plan.TranslationRawX == 256 && plan.TranslationRawY == 256 &&
            plan.TranslationRawZ == -40 && plan.DragonTargetSlotLogicalOffset == 0x5FC37 &&
            plan.DragonTargetBefore == 0 && plan.DragonTargetAfter == 4,
        "The exact camera/dragon-target transaction changed.");
    Require(plan.ChangedDataByteOffsets.Count == 349 &&
            plan.LogicalPatchWindowBytes == 1_393 && plan.ChangedLogicalWadBytes == 349 &&
            plan.ChangedLogicalExecutableBytes == 1 && plan.ChangedLogicalBytes == 350 &&
            plan.AffectedRawSectorLbas.SequenceEqual(new[] { 54_838, 54_845, 55_380, 55_381, 55_573 }) &&
            receipt.RebuiltRawSectorCount == 5 && receipt.ChangedRawSectorCount == 5 &&
            receipt.RawSectorDiffs.Select(diff => diff.RawSectorLba)
                .SequenceEqual(new[] { 54_838, 54_845, 55_380, 55_381, 55_573 }) &&
            receipt.RawSectorDiffs.Sum(diff => diff.PayloadChangedBytes) == 350 &&
            receipt.RawSectorDiffs.All(diff =>
                diff.HeaderChangedBytes == 0 && diff.SubheaderChangedBytes == 0 &&
                diff.ReservedChangedBytes == 0) &&
            receipt.RawSectorDiffs.Select(diff =>
                    (diff.EccPChangedBytes, diff.EccQChangedBytes))
                .SequenceEqual(new[] { (26, 48), (14, 34), (128, 103), (172, 104), (10, 20) }) &&
            IsPinnedOrPending(
                receipt.OutputImageSha256,
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ExpectedOutputImageSha256) &&
            (UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ExpectedChangedPhysicalImageBytes < 0 ||
             receipt.ChangedPhysicalImageBytes ==
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ExpectedChangedPhysicalImageBytes) &&
            IsPinnedOrPending(
                receipt.RawSectorDiffSha256,
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ExpectedRawSectorDiffSha256) &&
            IsPinnedOrPending(
                receipt.ConstructionPlanSha256,
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ExpectedConstructionPlanSha256) &&
            IsPinnedOrPending(
                receipt.LocationGuideSha256,
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ExpectedLocationGuideSha256),
        "The exact five-sector / 350-byte logical and physical boundary changed.");
    Require(plan.ExactT100T101T105AtomicRelocationVerified &&
            ...
            plan.RetailLevelsPreserved && !plan.MemoryCardsRequired &&
        ...
        "The fail-closed dragon/save-trigger plan flags changed.");
    Require(receipt.RollbackRecoveryVerified == expectRecovery &&
            ...
            receipt.CanonicalConstructionPlanVerified && receipt.CanonicalReceiptVerified &&
...
static bool IsPinnedOrPending(string actual, string expected) =>
    expected == "PENDING" || actual == expected;
    ...
    UnusedLevel65DragonSaveTriggerRuntimeCandidatePaths paths)
{
    ...
    Require(actual.SequenceEqual(expected.Order(StringComparer.Ordinal)),
        "The published dragon save-trigger candidate is not exactly seven files.");
    string metadata = await File.ReadAllTextAsync(paths.ConstructionPlanPath);
    using JsonDocument document = JsonDocument.Parse(metadata);
    JsonElement plan = document.RootElement;
    Require(plan.GetProperty("profileId").GetString() ==
                UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.ProfileId &&
            plan.GetProperty("rowPatches").EnumerateArray()
                .Select(value => value.GetProperty("trueIndex").GetInt32())
                .SequenceEqual(new[] { 100, 101, 105 }) &&
            plan.GetProperty("cameraFrameTrackPatch").GetProperty("frameCount").GetInt32() == 112 &&
            plan.GetProperty("dragonTargetBefore").GetInt32() == 0 &&
            plan.GetProperty("dragonTargetAfter").GetInt32() == 4 &&
            plan.GetProperty("changedLogicalBytes").GetInt32() == 350,
        "The published canonical construction plan changed.");
    ...
    string label)
{
    ...
    try
    {
        ...
        _ = await UnusedLevel65DragonSaveTriggerRuntimeCandidateExporter.VerifyPublishedAsync(paths);
    }
...
static async Task ExpectCoordinatedArtifactAndReceiptHashMutationFailureAsync(
    string artifactPath,
    Func<byte[], byte[]> mutate,
    UnusedLevel65DragonSaveTriggerRuntimeCandidatePaths paths,
    string receiptHashProperty,
    string label)
{
    byte[] originalArtifact = await File.ReadAllBytesAsync(artifactPath);
    byte[] changedArtifact = mutate(originalArtifact);
    byte[] originalReceipt = await File.ReadAllBytesAsync(paths.StaticReadbackReceiptPath);
    UnixFileMode? mode = GetUnixMode(artifactPath);
    Require(!changedArtifact.SequenceEqual(originalArtifact),
        $"The {label} fixture did not change the artifact.");
    bool rejected = false;
    try
    {
        await File.WriteAllBytesAsync(artifactPath, changedArtifact);
        RestoreUnixMode(artifactPath, mode);
        JsonObject receipt = JsonNode.Parse(Encoding.UTF8.GetString(originalReceipt))?.AsObject()
            ?? throw new InvalidOperationException("The coordinated-tamper receipt did not parse.");
        receipt[receiptHashProperty] = await HashFileAsync(artifactPath);
    ...
    {
        await File.WriteAllBytesAsync(artifactPath, originalArtifact);
        RestoreUnixMode(artifactPath, mode);
        await File.WriteAllBytesAsync(paths.StaticReadbackReceiptPath, originalReceipt);
    }
    Require(rejected, $"The {label} was accepted.");
    Require((await File.ReadAllBytesAsync(artifactPath)).SequenceEqual(originalArtifact) &&
            (await File.ReadAllBytesAsync(paths.StaticReadbackReceiptPath)).SequenceEqual(originalReceipt),
        $"The {label} fixture did not restore the exact artifact and receipt bytes.");
}
    ...
    UnusedLevel65DragonSaveTriggerRuntimeCandidateResult result)
{
    ...
    UnusedLevel65DragonSaveTriggerReceipt receipt =
        JsonSerializer.Deserialize<UnusedLevel65DragonSaveTriggerReceipt>(text, options)
...
static void RequireNoPublisherDebris(string publicationParent)
{
    if (!Directory.Exists(publicationParent))
        return;
    const string leaseFileName = ".unused-level-65-dragon-save-trigger-runtime-candidate-writer.lease";
    string[] ownedHiddenEntries = Directory.EnumerateFileSystemEntries(publicationParent)
        .Where(path => Path.GetFileName(path).StartsWith(
            ".unused-level-65-dragon-save-trigger-", StringComparison.Ordinal))
        .ToArray();
    Require(ownedHiddenEntries.All(path => Path.GetFileName(path) == leaseFileName),
        "dragon save-trigger operation debris remained after a completed path.");
    ...
    Require((attributes & (FileAttributes.Directory | FileAttributes.ReparsePoint)) == 0,
        "The persistent dragon save-trigger writer lease is not a regular file.");
}
