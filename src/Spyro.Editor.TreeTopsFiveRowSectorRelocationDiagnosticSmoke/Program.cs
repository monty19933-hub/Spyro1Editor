...
const int ExpectedOutputTextureComponentBytes = 0x3DD8;
...
const int ExpectedOutputUsedBytes = 0xA8044;
const int ExpectedSourceZeroTailBytes = 0x354;
const int ExpectedOutputZeroTailBytes = 0x7BC;
const int ExpectedTableGrowthBytes = 0x398;
const int ExpectedSectorGrowthBytes = 0x800;
const int ExpectedAdditionalBytesNeeded = 0x44;
...
const uint ExpectedFiveRowSceneEnd =
    RetailSceneEnd + ExpectedTableGrowthBytes;
const uint ExpectedFiveRowPolygonMargin =
    FixedLowerPolygonBuffer - ExpectedFiveRowSceneEnd;
...
string sourceSearchPath = Path.Combine(
    outputRoot,
    "treetops-count86-source-search.json");
string wadAnalysisPath = Path.Combine(
    outputRoot,
    "source-bound-wad-analysis.json");
string dormantPrefix = Path.Combine(
    outputRoot,
    "treetops-COUNT86-FIVE-ROWS-T81-T85-CLONE-T12-UNREFERENCED-SECTOR-RELOCATION-CONTROL");
string dormantFastPrefix =
    dormantPrefix + "-FAST-ENTRY";
string visiblePrefix = Path.Combine(
    outputRoot,
    "treetops-COUNT86-FIVE-ROWS-T81-T85-CLONE-T12-FIVE-VISIBLE-FACES-SECTOR-RELOCATION-DIAGNOSTIC");
...
string proofPath = Path.Combine(
    outputRoot,
    "tree-tops-count86-five-row-sector-relocation-diagnostic-proof.json");
string checklistPath = Path.Combine(
    outputRoot,
    "tree-tops-count86-five-row-runtime-checklist.md");
...
Assert(
    ...
    "Tree Tops count86 diagnostics are guarded to the exact clean USA retail BIN.");
...
Assert(
    ...
    binding.ExpectedLevelDataSha256.Equals(
        ...
        StringComparison.OrdinalIgnoreCase),
    "Tree Tops count86 source binding changed.");
...
Assert(
    ...
    "Tree Tops no longer exposes the exact normal-checked five-row +0x800 sector-relocation boundary.");
    ...
    assignments
        ...
        .SequenceEqual(
            Enumerable.Range(
                ...
                AppendedRowCount)),
    "The count86 face assignment matrix is not five unique faces mapped one-to-one to T81-T85.");
...
int saved = await TerrainEditStore.SaveAsync(
    editsPath,
    editedFaces,
    "Tree Tops count86/T81-T85 five-row sector-relocation diagnostic");
Assert(
    saved == AppendedRowCount,
    $"Saved {saved} count86 face edit(s), expected {AppendedRowCount}.");
...
Assert(
    ...
    ordinaryPlan.SkippedEdits.All(item =>
        item.Equals(
            ...
            StringComparison.Ordinal)),
    "The ordinary Tree Tops plan was not exactly five source-bound texture-word patches.");
...
NativeTerrainTexturePackedAppendRecord[] packedRows =
    assignments.Select(assignment =>
        new NativeTerrainTexturePackedAppendRecord(
            StableEditId:
                $"research-only:treetops:count86:{assignment.RuntimeKey}:T{assignment.TargetTextureId}:clone-native-T12",
...
Assert(
    ...
    CountByteDifferences(
        ...
        AppendedRowCount,
    "The dormant/visible count86 plans are not page-patch-free or do not differ by exactly five face bytes.");
foreach (FaceAssignment assignment in assignments)
{
    ...
    Assert(
        ...
        $"Tree Tops {assignment.RuntimeKey} did not rebase to its exact count86 output offset.");
}
...
Assert(
    ...
    CountFileByteDifferences(
        ...
        visibleExport.OutputImagePath) ==
        AppendedRowCount,
    "The two count86 final BINs changed texture pages or differ by more than the five visible face bytes.");
...
Assert(
    CountFileByteDifferences(
        ...
        AppendedRowCount,
    "The two count86 fast-entry BINs differ by more than the five selected face bytes.");
...
var proof = new
{
    ...
    Status =
        "COUNT83 AND COUNT85 REPORTED LOADING; COUNT86 DORMANT/VISIBLE OFFLINE BUILD AND EXACT READBACK PASSED; COUNT86 DUCKSTATION TESTS PENDING",
    ResearchOnly = true,
    ...
    ReportedDuckStationPrerequisites = new
    {
        ...
        EvidenceScope =
            "Interactive user report; this authorizes only the first sector-relocation research test."
    },
    Source = new
    {
    ...
    StructuralReadback = new
    {
        ...
        SceneEnd =
            $"0x{ExpectedFiveRowSceneEnd:X8}",
        PolygonBufferMargin =
            $"0x{ExpectedFiveRowPolygonMargin:X}",
        ...
        VisibleShiftedSuffixPreservedExceptFiveSourceBoundFaceBytes = true
    },
    ...
    RuntimeBoundary =
        "Cold-boot the dormant fast-entry CUE first, then the visible fast-entry CUE. Passing offline readback is not DuckStation evidence and does not promote Tree Tops capacity."
};
...
string checklist = $"""
    # Tree Tops count86 five-row sector-relocation runtime checklist

    Research only. This is the first test beyond Tree Tops' native fixed tail. It installs exactly five complete T12-clone rows (T81-T85), not the old 46-row batch.
    ...
    1. Fully stop the currently running game and cold-boot `{Path.GetFileName(dormantFast.CuePath)}`.
    2. Confirm Tree Tops loads, its music continues, Spyro can move, and nearby actors behave normally.
    3. Quit that boot completely.
    4. Cold-boot `{Path.GetFileName(visibleFast.CuePath)}`.
    5. Confirm Tree Tops again loads, its music continues, Spyro can move, and nearby actors behave normally.
    6. The five selected native T12 faces intentionally look unchanged: they now reference byte-identical T81-T85 clones. The signal is loading and stable gameplay, not a visual color change.
    7. If either image freezes, note which CUE, whether the screen blinks, whether music begins, and whether the dormant or visible image failed. Stop there.

    ## Exact construction

    - Texture count: 81 -> 86.
    - Texture rows: exactly five appended complete T12 clones.
    ...
    - Dormant/visible difference: exactly five terrain texture bytes.
    - Fast-entry difference from its corresponding control: exactly eight guarded executable bytes.

    Passing these two CUEs proves only this exact five-row Tree Tops recipe. Normal Create BIN remains unchanged until runtime evidence is recorded and higher thresholds are tested separately.
    """;
await File.WriteAllTextAsync(
    checklistPath,
...
Console.WriteLine(
    "Tree Tops count86 five-row sector-relocation diagnostics: PASSED OFFLINE");
Console.WriteLine(
    $"- Source: {sourceImagePath}");
Console.WriteLine(
    $"- Rows: exactly T81-T85, each a complete byte-identical native T12 clone.");
Console.WriteLine(
...
void AssertRetailSourcePreserved()
{
    Assert(
        ...
        Sha256File(sourceImagePath).Equals(
            sourceSha256,
            StringComparison.OrdinalIgnoreCase),
        "The count86 diagnostic changed the immutable retail source.");
}
    ...
    int expectedFacePatchCount)
{
    ...
    Assert(
        ...
        "The count86 sector relocation changed its source, WAD, entry, LBA, policy, or no-page-patch contract.");
    Assert(
        ...
        plan.RelocatedLevelDataPatchProofs.Count ==
            expectedFacePatchCount,
        "The count86 append changed its exact row, component, suffix, tail, or face-rebase contract.");
    Assert(
        ...
        append.ShiftedComponents.All(item =>
            ...
            item.ComposedSourceSha256.Equals(
                item.OutputSha256,
                StringComparison.OrdinalIgnoreCase)),
        "A count86 suffix component did not shift by +0x398 byte-identically.");
    Assert(
        ...
        append.ResolvedRecords.All(item =>
            ...
            item.HighDetailSha256.Equals(
                Sha256Bytes(sourceT12High),
                StringComparison.OrdinalIgnoreCase)),
        "The count86 plan did not assign exactly T81-T85 as complete native T12 clones.");
    for (int textureId = 0;
            ...
            SliceHighRow(
                ...
                .SequenceEqual(
                    SliceHighRow(
                        ...
                        OutputTextureCount,
                        textureId)),
            $"Native Tree Tops T{textureId} was overridden while building count86.");
    }
    ...
    Assert(
        ExpectedFiveRowSceneEnd ==
            0x8018688C &&
        ExpectedFiveRowPolygonMargin ==
            0x1324 &&
        ExpectedFiveRowSceneEnd <
            FixedLowerPolygonBuffer,
        "The count86 scene-end/polygon-buffer margin changed.");
}
    ...
    IReadOnlyList<FaceExpectation> expectedFaces)
{
    ...
    Assert(
        ...
        FileLength(imagePath) ==
            sourceLength,
        "The count86 final image changed the disc layout or physical image length.");
    ...
    Assert(
        ...
        "The final level-data subfile does not exactly match the composed count86 payload.");
    ...
    Assert(
        sourceTexturePagesSha256.Equals(
            ...
            StringComparison.Ordinal),
        "The count86 diagnostic changed Tree Tops texture-page bytes despite declaring no page/global repack.");
    ...
    Assert(
        slots.Count ==
            OutputTextureCount &&
        slots[^1].TextureId == 85,
        $"The final count86 decoder found {slots.Count} texture rows.");
    ...
    Assert(
        ...
        "The final count86 image failed runtime-target readback for T81-T85.");
    ...
    IReadOnlyList<FaceExpectation> expectedFaces)
{
    ...
    Assert(
        ...
        $"Fast-entry clone differs from its count86 control by {controlDifferences} byte(s), expected exactly eight guarded executable bytes.");
