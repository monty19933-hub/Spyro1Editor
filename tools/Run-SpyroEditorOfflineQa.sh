...
Options:
  ...
  --stone-hill-town-square-edited-donor-candidate-smoke
      Build the corrected pending-runtime V5 edited Town Square donor CUE from
      the exact runtime-proven display-identity control. Moves Town Square T21's
      X coordinate while preserving its native +0x4A = FF placement sentinel;
      guards the single rebased Stone Hill-slot write, final known hash, original
      donor data, evidence files, determinism, and unsafe-patch rejection;
      launches no emulator.
...
if [[ "$STONEHILL_TOWNSQUARE_EDITED_DONOR_CANDIDATE_SMOKE_ONLY" -eq 1 ]]; then
    echo "Building focused V5 edited Town Square donor X-only native-FF candidate smoke..."
    dotnet build "$STONEHILL_TOWNSQUARE_EDITED_DONOR_SMOKE_PROJECT" --configuration "$CONFIGURATION" --nologo
    echo
    echo "Running focused V5 edited Town Square donor X-only native-FF candidate smoke..."
